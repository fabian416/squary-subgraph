...
const DatePicker = ({ dates, setDates }) => {
  ...
  return (
    <material_1.Box position="absolute" zIndex={2} top={30} right={320} border="1px solid white">
      <lab_1.LocalizationProvider dateAdapter={moment_1.default}>
        <lab_1.StaticDatePicker
          displayStaticWrapperAs="desktop"
          onChange={(newVal) => {
            if (newVal) {
              setDates((prev) => [...prev, newVal].sort((a, b) => (a.isBefore(b) ? -1 : 1)));
            }
          }}
          value={datesValue}
          renderDay={(day, _selectedDates, pickersDayProps) => {
            return (
              <div
                key={day.format("l")}
                onClick={() => {
                  if (dates.map((date) => date.format("l")).includes(day.format("l"))) {
                    setDates(dates.filter((date) => date.format("l") !== day.format("l")));
                  }
                }}
              >
                <lab_1.PickersDay
