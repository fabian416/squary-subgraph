...
function updateUsageMetrics(event, fromAddress, usageType) {
  ...
  const usageMetricsDaily = (0, getters_1.getOrCreateUsageMetricDailySnapshot)(
    event
  );
...
function updatePoolMetrics(event) {
  ...
  const poolMetricsDaily = (0, getters_1.getOrCreateLiquidityPoolDailySnapshot)(
    event
  );
  const poolMetricsHourly = (0,
  getters_1.getOrCreateLiquidityPoolHourlySnapshot)(event);
