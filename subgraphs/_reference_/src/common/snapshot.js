...
function createProtocolSnapshots(event, protocol) {
  ...
  const snapshotDayID =
    protocol.lastUpdateTimestamp.toI32() / constants_1.SECONDS_PER_DAY;
  const snapshotHourID =
    protocol.lastUpdateTimestamp.toI32() / constants_1.SECONDS_PER_HOUR;
...
function createLiquidityPoolSnapshots(event, pool) {
  ...
  const hourID = event.block.timestamp.toI32() / constants_1.SECONDS_PER_HOUR;
  const snapshotDayID =
    pool.lastUpdateTimestamp.toI32() / constants_1.SECONDS_PER_DAY;
  const snapshotHourID =
    pool.lastUpdateTimestamp.toI32() / constants_1.SECONDS_PER_HOUR;
  if (snapshotDayID != dayID) {
...
function takeUsageHourlySnapshot(event, protocol, hour) {
  ...
  const previousSnapshot = schema_1.UsageMetricsHourlySnapshot.load(
...
function takeLiquidityPoolHourlySnapshot(event, pool, hour) {
  const snapshot = new schema_1.LiquidityPoolHourlySnapshot(
    pool.id.concatI32(hour)
  );
  const previousSnapshot = schema_1.LiquidityPoolHourlySnapshot.load(
...
function takeLiquidityPoolDailySnapshot(event, pool, day) {
  const snapshot = new schema_1.LiquidityPoolDailySnapshot(
    pool.id.concatI32(day)
  );
  const previousSnapshot = schema_1.LiquidityPoolDailySnapshot.load(
