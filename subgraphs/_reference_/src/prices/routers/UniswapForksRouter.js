...
function getPriceFromRouter(token0Address, token1Address, block = null) {
  ...
  for (let idx = 0; idx < routerAddresses.length; idx++) {
    ...
    const uniswapForkRouter = UniswapRouter_1.UniswapRouter.bind(
      router.address
    );
  ...
  const amountOutBigDecimal = amountOut
    ...
    .toBigDecimal();
  const priceFromRouter = types_1.CustomPriceType.initialize(
    amountOutBigDecimal,
    config.usdcTokenDecimals().toI32(),
...
function getLpTokenTotalLiquidityUsdc(tokenAddress, block = null) {
  ...
  if (reservesCall.reverted) return new types_1.CustomPriceType();
...
function getLpTokenLiquidityUsdc(lpAddress, wethAddress, block = null) {
  ...
  if (token1Call.reverted) return new types_1.CustomPriceType();
