...
class ProtocolSnapshot {
  ...
  takeSnapshots() {
    const snapshotDayID =
      this.protocol.lastUpdateTimestamp.toI32() / constants_1.SECONDS_PER_DAY;
    const snapshotHourID =
      this.protocol.lastUpdateTimestamp.toI32() / constants_1.SECONDS_PER_HOUR;
  ...
  takeFinancialsDailySnapshot(day) {
    ...
    const previousSnapshot = schema_1.FinancialsDailySnapshot.load(
      graph_ts_1.Bytes.fromI32(this.protocol.lastSnapshotDayID)
    );
    snapshot.day = day;
  ...
  takeUsageHourlySnapshot(hour) {
    ...
    const previousSnapshot = schema_1.UsageMetricsHourlySnapshot.load(
      graph_ts_1.Bytes.fromI32(this.protocol.lastSnapshotHourID)
    );
    snapshot.protocol = this.protocol.id;
    snapshot.hour = hour;
    snapshot.blockNumber = this.event.block.number;
