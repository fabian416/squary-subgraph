...
class Account {
  ...
  getAmountUSD(tokens, amounts) {
    ...
    for (let idx = 0; idx < tokens.length; idx++) {
      const token = this.tokens.getOrCreateToken(
        graph_ts_1.Address.fromBytes(tokens[idx])
      );
  ...
  trackActivity(activityType, liquidator = null) {
    ...
    const generalActivity = {
      daily: this.isActiveByActivityID(
        graph_ts_1.Bytes.fromUTF8(generalDailyID)
      ),
      hourly: this.isActiveByActivityID(
        graph_ts_1.Bytes.fromUTF8(generalHourlyID)
      ),
    };
    ...
    const typeActivity = {
      ...
      hourly: this.isActiveByActivityID(
        graph_ts_1.Bytes.fromUTF8(typeHourlyID)
      ),
    };
  ...
  countLiquidator(liquidator) {
    ...
    entity.save();
    this.trackActivity(enums_1.ActivityType.LIQUIDATOR, liquidator);
  }
  /**
