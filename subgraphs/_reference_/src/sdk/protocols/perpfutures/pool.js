...
class Pool {
  ...
  initialize(name, symbol, inputTokens, outputToken, oracle = null) {
    ...
    this.pool.fundingrate = new Array(inputTokens.length).fill(
    ...
    this.pool.cumulativeVolumeByTokenAmount = new Array(
      inputTokens.length
    ).fill(constants.BIGINT_ZERO);
    this.pool.cumulativeVolumeByTokenUSD = new Array(inputTokens.length).fill(
      constants.BIGDECIMAL_ZERO
    );
    this.pool.cumulativeInflowVolumeUSD = constants.BIGDECIMAL_ZERO;
    this.pool.cumulativeInflowVolumeByTokenAmount = new Array(
      inputTokens.length
    ).fill(constants.BIGINT_ZERO);
    this.pool.cumulativeInflowVolumeByTokenUSD = new Array(
      inputTokens.length
    ).fill(constants.BIGDECIMAL_ZERO);
    this.pool.cumulativeClosedInflowVolumeUSD = constants.BIGDECIMAL_ZERO;
    this.pool.cumulativeClosedInflowVolumeByTokenAmount = new Array(
      inputTokens.length
    ).fill(constants.BIGINT_ZERO);
    this.pool.cumulativeClosedInflowVolumeByTokenUSD = new Array(
      inputTokens.length
    ).fill(constants.BIGDECIMAL_ZERO);
    this.pool.cumulativeOutflowVolumeUSD = constants.BIGDECIMAL_ZERO;
    this.pool.cumulativeOutflowVolumeByTokenAmount = new Array(
      inputTokens.length
    ).fill(constants.BIGINT_ZERO);
    this.pool.cumulativeOutflowVolumeByTokenUSD = new Array(
      inputTokens.length
    ).fill(constants.BIGDECIMAL_ZERO);
    this.pool.inputTokenBalances = new Array(inputTokens.length).fill(
      constants.BIGINT_ZERO
    );
    this.pool.inputTokenWeights = new Array(inputTokens.length).fill(
      constants.BIGDECIMAL_ZERO
    );
  ...
  getOutputToken() {
    if (!this.pool.outputToken) return graph_ts_1.Bytes.empty();
  ...
  setRewardEmissions(type, token, amount) {
    ...
    if (this.pool.rewardTokens.includes(rToken.id)) {
      ...
      this.pool.rewardTokenEmissionsAmount = (0, arrays_1.updateArrayAtIndex)(
        this.pool.rewardTokenEmissionsAmount,
        amount,
        index
      );
      this.pool.rewardTokenEmissionsUSD = (0, arrays_1.updateArrayAtIndex)(
        this.pool.rewardTokenEmissionsUSD,
        amountUSD,
        index
      );
    ...
    amountsUSD = (0, arrays_1.sortArrayByReference)(
      newOrder,
      tokens,
      amountsUSD
    );
