...
class DataManager {
  constructor(marketID, inputToken, event, protocolData) {
    ...
    this.event = event;
    // load snapshots
    this.snapshots = new snapshots_1.SnapshotManager(
      event,
      this.protocol,
      this.market
    );
  ...
  createDeposit(
    ...
    const position = new position_1.PositionManager(
  ...
  createBorrow(
    ...
    amountUSD,
    newBalance,
    tokenPriceUSD, // used for different borrow token in CDP
    interestType = null,
    isIsolated = false,
    principal = null
  ) {
  ...
  createRepay(
    ...
    if (!positionID) {
      graph_ts_1.log.error(
        "[createRepay] positionID is null for market: {} account: {}",
        [this.market.id.toHexString(), account.toHexString()]
      );
      return null;
    }
  ...
  createLiquidate(
    ...
    newBorrowerBalance,
    interestType = null,
    subtractBorrowerPosition = true,
    principal = null
  ) {
    const positions = []; // positions touched by this liquidation
    ...
    const collateralPosition = new position_1.PositionManager(
    ...
    if (subtractBorrowerPosition) {
      const debtMarket = schema_1.Market.load(debtTokenId);
      if (!debtMarket) {
        graph_ts_1.log.error("[createLiquidate] market {} not found", [
          debtTokenId.toHexString(),
        ]);
        return null;
      }
      const borrowerPosition = new position_1.PositionManager(
        liquidateeAccount.getAccount(),
        debtMarket,
        constants_1.PositionSide.BORROWER,
        interestType
  ...
  createTransfer(
    ...
    const transferrerPosition = new position_1.PositionManager(
    ...
    transferrerPosition.subtractPosition(
      ...
      this.protocol,
      senderNewBalance,
      constants_1.TransactionType.TRANSFER,
      this.market.inputTokenPriceUSD,
      senderPrincipal
    );
    ...
    const receiverPosition = new position_1.PositionManager(
    ...
    const transfer = new schema_1.Transfer(
      this.event.transaction.hash
        .concatI32(this.event.logIndex.toI32())
  ...
  createFlashloan(asset, account, amount, amountUSD) {
    ...
    const flashloan = new schema_1.Flashloan(
      this.event.transaction.hash
        .concatI32(this.event.logIndex.toI32())
    ...
    flashloan.save();
    this.updateTransactionData(
      constants_1.TransactionType.FLASHLOAN,
      amount,
      amountUSD
    );
  ...
  updateRewards(rewardData) {
    ...
    for (let i = 0; i < rewardTokens.length; i++) {
      ...
      if (index < 0) {
        ...
        rewardTokens = (0, constants_2.insert)(
          rewardTokens,
          rewardData.rewardToken.id,
          i
        );
        rewardTokenEmissionsAmount = (0, constants_2.insert)(
          rewardTokenEmissionsAmount,
          rewardData.rewardTokenEmissionsAmount,
          i
        );
        rewardTokenEmissionsUSD = (0, constants_2.insert)(
          rewardTokenEmissionsUSD,
          rewardData.rewardTokenEmissionsUSD,
          i
        );
  ...
  updateMarketAndProtocolData(
    ...
    newVariableBorrowBalance = null,
    newStableBorrowBalance = null,
    ...
    for (let i = 0; i < marketList.length; i++) {
      ...
      if (!_market) {
        graph_ts_1.log.error(
          "[updateMarketAndProtocolData] Market not found: {}",
  ...
  updateUsageData(transactionType, account) {
    this.market.cumulativeUniqueUsers += (0, constants_1.activityCounter)(
      account,
      transactionType,
      false,
      0,
    ...
    if (transactionType == constants_1.TransactionType.DEPOSIT) {
      this.market.cumulativeUniqueDepositors += (0,
      ...
      this.protocol.cumulativeUniqueDepositors += (0,
      constants_1.activityCounter)(account, transactionType, true, 0);
    }
    if (transactionType == constants_1.TransactionType.BORROW) {
      this.market.cumulativeUniqueBorrowers += (0, constants_1.activityCounter)(
        account,
        transactionType,
        true,
        0,
      ...
      this.protocol.cumulativeUniqueBorrowers += (0,
      constants_1.activityCounter)(account, transactionType, true, 0);
    }
    if (transactionType == constants_1.TransactionType.LIQUIDATOR) {
      this.market.cumulativeUniqueLiquidators += (0,
      constants_1.activityCounter)(
        account,
        transactionType,
        true,
      ...
      this.protocol.cumulativeUniqueLiquidators += (0,
      constants_1.activityCounter)(account, transactionType, true, 0);
    }
    if (transactionType == constants_1.TransactionType.LIQUIDATEE) {
      this.market.cumulativeUniqueLiquidatees += (0,
      constants_1.activityCounter)(
        account,
        transactionType,
        true,
      ...
      this.protocol.cumulativeUniqueLiquidatees += (0,
      constants_1.activityCounter)(account, transactionType, true, 0);
    }
    if (transactionType == constants_1.TransactionType.TRANSFER)
      this.market.cumulativeUniqueTransferrers += (0,
      constants_1.activityCounter)(
        account,
        transactionType,
        true,
    ...
    if (transactionType == constants_1.TransactionType.FLASHLOAN)
      this.market.cumulativeUniqueFlashloaners += (0,
      constants_1.activityCounter)(
        account,
        transactionType,
        true,
    ...
    if (details.sources.length == 0) {
      ...
      if (sources.includes(associatedSource)) {
        ...
        for (let i = 0; i < sourcesSorted.length; i++) {
          ...
          amountsUSDSorted = (0, constants_2.insert)(
            amountsUSDSorted,
            amountsUSD[idx]
          );
        }
