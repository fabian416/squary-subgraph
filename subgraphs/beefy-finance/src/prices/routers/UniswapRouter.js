...
const UniswapFeeRouter_1 = require("../../../generated/Standard/UniswapFeeRouter");
const graph_ts_1 = require("@graphprotocol/graph-ts");
const UniswapRouter_1 = require("../../../generated/Standard/UniswapRouter");
function isLpToken(tokenAddress, network) {
  if (tokenAddress == constants.WHITELIST_TOKENS_MAP.get(network).get("ETH")) {
    return false;
  }
  const lpToken = UniswapPair_1.UniswapPair.bind(tokenAddress);
...
function getPriceFromRouterUsdc(tokenAddress, network) {
  if (tokenAddress == constants.WHITELIST_TOKENS_MAP.get(network).get("USDC")) {
...
function getPriceFromRouter(token0Address, token1Address, network) {
  ...
  if (routerAddressV1) {
    ...
    if (amountOutArray.reverted) {
      const uniswapFeeRouter =
        UniswapFeeRouter_1.UniswapFeeRouter.bind(routerAddressV1);
      amountOutArray = uniswapFeeRouter.try_getAmountsOut(
        amountIn,
        path,
        graph_ts_1.BigInt.fromI32(3000)
      );
      if (amountOutArray.reverted) {
        const uniswapFeeRouter =
          UniswapFeeRouter_1.UniswapFeeRouter.bind(routerAddressV1);
