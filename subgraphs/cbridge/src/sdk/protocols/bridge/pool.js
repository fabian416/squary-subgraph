...
class PoolManager {
  ...
  loadPool(id, onCreate = null, aux1 = null, aux2 = null) {
...
class Pool {
  ...
  getInputToken() {
    return this.tokens.getOrCreateToken(
      graph_ts_1.Address.fromBytes(this.pool.inputToken)
    );
  }
  initialize(name, symbol, type, inputToken) {
    ...
    this.tokens.registerSupportedToken(
      graph_ts_1.Address.fromBytes(inputToken.id)
    );
  }
  ...
  routeIDFromCrosschainToken(token) {
    ...
    return graph_ts_1.Bytes.fromUTF8(
  ...
  getInputTokenAmountPrice(amount) {
    ...
    token.save();
    return amount
      .divDecimal((0, numbers_1.exponentToBigDecimal)(token.decimals))
      .times(price);
  }
  /**
   ...
   */
  addInputTokenBalance(amount, updateMetrics = true) {
  ...
  refreshOutputTokenPriceUSD() {
    ...
    const token = this.tokens.getOrCreateToken(
      graph_ts_1.Address.fromBytes(this.pool.outputToken)
    );
  ...
  trackTransfer(transfer, route, eventType) {
    ...
    if (this.pool.type == enums_1.BridgePoolType.BURN_MINT) {
      ...
      this.pool.type == enums_1.BridgePoolType.LIQUIDITY ||
  ...
  trackWithdraw(withdraw) {
    ...
    this.addInputTokenBalance(
      withdraw.amount.times(constants_1.BIGINT_MINUS_ONE)
    );
  }
}
exports.Pool = Pool;
