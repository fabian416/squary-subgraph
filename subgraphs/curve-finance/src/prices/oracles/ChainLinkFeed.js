...
const ChainLinkContract_1 = require("../../../generated/templates/PoolTemplate/ChainLinkContract");
function getTokenPriceUSDC(tokenAddr, block = null) {
  const config = utils.getConfig();
  const contractAddress = utils.getContract(config.chainLink(), block);
  if (!contractAddress) return new types_1.CustomPriceType();
  const chainLinkContract =
    ChainLinkContract_1.ChainLinkContract.bind(contractAddress);
  const result = chainLinkContract.try_latestRoundData(
    tokenAddr,
    constants.CHAIN_LINK_USD_ADDRESS
  );
  if (!result.reverted) {
    const decimals = utils.readValue(
