...
const InchOracleContract_1 = require("../../../generated/templates/PoolTemplate/InchOracleContract");
function getTokenPriceUSDC(tokenAddr, block = null) {
  ...
  const inchOracleContract =
    InchOracleContract_1.InchOracleContract.bind(contractAddress);
  for (let i = 0; i < constants.STABLE_TOKENS.length; i++) {
    ...
    if (constants.STABLE_TOKENS[i] == "WETH") {
      const wethToken = (0, initializers_1.getOrCreateToken)(
        dstToken.address,
        block
      );
    ...
    return types_1.CustomPriceType.initialize(
      tokenPrice.times(utils.exponentToBigDecimal(srcTokenDecimals.toI32())),
      dstToken.decimals + constants.DEFAULT_DECIMALS.toI32(),
