...
exports.getPriceUsdc =
  exports.cryptoPoolUnderlyingTokensAddressesByPoolAddress =
  exports.cryptoPoolTokenAmountUsdc =
  exports.cryptoPoolLpTotalValueUsdc =
  exports.cryptoPoolLpPriceUsdc =
  exports.getPriceUsdcRecommended =
  exports.getVirtualPrice =
  exports.getPreferredCoinFromCoins =
  exports.getUnderlyingCoinFromPool =
  exports.getBasePrice =
  exports.getCurvePriceUsdc =
  exports.isPoolCryptoPool =
  exports.isLpCryptoPool =
  exports.getPoolFromLpToken =
  exports.isCurveLpToken =
    void 0;
const __1 = require("..");
...
const CurvePool_1 = require("../../../generated/BasicWeeklyCalls/CurvePool");
const CurveRegistry_1 = require("../../../generated/BasicWeeklyCalls/CurveRegistry");
...
function getPoolFromLpToken(lpAddress, block) {
  ...
  for (let idx = 0; idx < curveRegistryAdresses.length; idx++) {
    ...
    const curveRegistryContract = CurveRegistry_1.CurveRegistry.bind(
...
function isLpCryptoPool(lpAddress, block) {
  const poolAddress = getPoolFromLpToken(lpAddress, block);
...
function getCurvePriceUsdc(lpAddress, block) {
  ...
  return types_1.CustomPriceType.initialize(
    priceUsdc,
    decimalsAdjustment.plus(constants.DEFAULT_DECIMALS).toI32(),
    constants.OracleType.CURVE_ROUTER
  );
}
exports.getCurvePriceUsdc = getCurvePriceUsdc;
function getBasePrice(lpAddress, block) {
  const poolAddress = getPoolFromLpToken(lpAddress, block);
...
function getUnderlyingCoinFromPool(poolAddress, block) {
  const config = utils.getConfig();
  const curveRegistryAdresses = config.curveRegistry();
  for (let idx = 0; idx < curveRegistryAdresses.length; idx++) {
    const curveRegistry = curveRegistryAdresses[idx];
...
function getVirtualPrice(curveLpTokenAddress, block) {
  const config = utils.getConfig();
  const curveRegistryAdresses = config.curveRegistry();
  for (let idx = 0; idx < curveRegistryAdresses.length; idx++) {
    const curveRegistry = curveRegistryAdresses[idx];
...
function cryptoPoolLpTotalValueUsdc(lpAddress, block) {
  const poolAddress = getPoolFromLpToken(lpAddress, block);
  ...
  for (
    ...
    const tokenValueUsdc = cryptoPoolTokenAmountUsdc(
      ...
      graph_ts_1.BigInt.fromI32(tokenIdx),
      block
    );
...
function cryptoPoolTokenAmountUsdc(poolAddress, tokenAddress, tokenIdx, block) {
  const poolContract = CurvePool_1.CurvePool.bind(poolAddress);
...
function cryptoPoolUnderlyingTokensAddressesByPoolAddress(poolAddress) {
  ...
  const coins = [];
  while (idx >= 0) {
    const coin = utils.readValue(
      poolContract.try_coins(graph_ts_1.BigInt.fromI32(idx)),
      constants.NULL.TYPE_ADDRESS
    );
