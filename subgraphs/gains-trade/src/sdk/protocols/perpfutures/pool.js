...
class Pool {
    ...
    addInputToken(tokenAddress) {
        if (!this.pool.inputTokens.includes(tokenAddress)) {
            this.pool.inputTokens = (0, arrays_1.addToArrayAtIndex)(this.pool.inputTokens, tokenAddress);
            this.pool.inputTokenBalances = (0, arrays_1.addToArrayAtIndex)(this.pool.inputTokenBalances, constants.BIGINT_ZERO);
            this.pool.inputTokenWeights = (0, arrays_1.addToArrayAtIndex)(this.pool.inputTokenWeights, constants.BIGDECIMAL_ZERO);
            this.save();
        }
        return;
    }
    ...
    // TODO
    updateOpenInterestByToken(pairIndex, token, oiLong, oiShort) {
        let oiHelper = schema_1._OpenInterestHelper.load(graph_ts_1.Bytes.fromI32(pairIndex.toI32()));
        if (!oiHelper) {
            oiHelper = new schema_1._OpenInterestHelper(graph_ts_1.Bytes.fromI32(pairIndex.toI32()));
    ...
    refreshOutputTokenPriceUSD() {
        ...
        const price = (0, numbers_2.safeDivide)(this.pool.totalValueLockedUSD, (0, numbers_1.bigIntToBigDecimal)(this.pool.outputTokenSupply, token.decimals));
     ...
     */
    updateFundingRateByToken(pairIndex, token, fundingRatePerDay) {
        let frHelper = schema_1._FundingRateHelper.load(graph_ts_1.Bytes.fromI32(pairIndex.toI32()));
        if (!frHelper) {
            frHelper = new schema_1._FundingRateHelper(graph_ts_1.Bytes.fromI32(pairIndex.toI32()));
     ...
     * @see Account
     */
    closePosition(positionSide, isExistingOpenPosition = true) {
    ...
    addDailyRewards(type, token, amount) {
