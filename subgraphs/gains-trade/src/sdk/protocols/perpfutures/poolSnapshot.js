...
class PoolSnapshot {
    ...
    takeSnapshots() {
        if (!this.isInitialized())
            return;
        if (!this.pool._lastUpdateTimestamp)
            return;
    ...
    getDailyRewardsHelper() {
        const dayID = this.pool._lastUpdateTimestamp.toI32() / constants.SECONDS_PER_DAY;
        return getOrCreateDailyRewardsHelper(dayID, this.pool);
    }
}
exports.PoolSnapshot = PoolSnapshot;
function getOrCreateDailyRewardsHelper(day, pool) {
    let entity = schema_1._DailyRewardsHelper.load(pool.id.concatI32(day));
    if (!entity) {
        entity = new schema_1._DailyRewardsHelper(pool.id.concatI32(day));
        entity.dailyRewardsAmount = constants.BIGINT_ZERO;
        entity.prevDayRewardsAmount = constants.BIGINT_ZERO;
    }
    const prevDayEntity = schema_1._DailyRewardsHelper.load(pool.id.concatI32(day - 1));
    if (prevDayEntity) {
