...
const UniswapRouter_1 = require("../../../generated/templates/Hypervisor/UniswapRouter");
...
function getPriceFromRouter(token0Address, token1Address, network) {
    ...
    if (pathOverride) {
        path = pathOverride;
        numberOfJumps = graph_ts_1.BigInt.fromI32(path.length - 1);
    }
    else {
    ...
    let amountIn = graph_ts_1.BigInt.fromI32(10).pow(token0Decimals.toI32());
    ...
    if (routerAddressV1) {
        ...
        if (amountOutArray.reverted && routerAddressV2) {
            const uniswapRouterV2 = UniswapRouter_1.UniswapRouter.bind(routerAddressV2);
        ...
        let amountOutBigDecimal = amountOut
            ...
            .toBigDecimal();
        return types_1.CustomPriceType.initialize(amountOutBigDecimal, constants.USDC_DECIMALS_MAP.get(network).toI32());
    }
    return new types_1.CustomPriceType();
}
...
function getLpTokenPriceUsdc(tokenAddress, network) {
    ...
    let totalSupply = utils.readValue(uniSwapPair.try_totalSupply(), constants.BIGINT_ZERO);
...
function getLpTokenTotalLiquidityUsdc(tokenAddress, network) {
    const uniSwapPair = UniswapPair_1.UniswapPair.bind(tokenAddress);
    let token0Address = utils.readValue(uniSwapPair.try_token0(), constants.ZERO_ADDRESS);
    let token1Address = utils.readValue(uniSwapPair.try_token1(), constants.ZERO_ADDRESS);
    ...
    let reserves = utils.readValue(uniSwapPair.try_getReserves(), constants.UNISWAP_DEFAULT_RESERVE_CALL);
