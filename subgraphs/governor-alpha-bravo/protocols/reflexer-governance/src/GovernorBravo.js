...
const GovernorBravo_1 = require("../../../generated/GovernorBravo/GovernorBravo");
...
function handleProposalCreated(event) {
    ...
    for (let i = 0; i < event.params.targets.length; i++) {
        values[i] = constants_1.BIGINT_ZERO;
        signatures[i] = "";
    }
    // FIXME: Prefer to use a single object arg for params
    // e.g.  { proposalId: event.params.proposalId, proposer: event.params.proposer, ...}
    // but graph wasm compilation breaks for unknown reasons
    (0, handlers_1._handleProposalCreated)(event.params.id.toString(), event.params.proposer.toHexString(), event.params.targets, values, signatures, event.params.calldatas, event.params.startBlock, event.params.endBlock, event.params.description, quorumVotes, event);
}
...
function getLatestProposalValues(proposalId, contractAddress) {
    ...
    if (proposal.state == constants_1.ProposalState.PENDING) {
        const contract = GovernorBravo_1.GovernorBravo.bind(contractAddress);
...
function getGovernanceFramework(contractAddress) {
    ...
    if (!governanceFramework) {
        ...
        const contract = GovernorBravo_1.GovernorBravo.bind(graph_ts_1.Address.fromString(contractAddress));
