...
class Account {
     ...
     */
    nonEVMTransferOut(pool, route, destination, amount, transactionID = null, updateMetrics = true) {
        this.countTransferOut();
        return this.transfer(pool, route, destination, amount, true, transactionID, updateMetrics);
    }
    /**
     * Creates a BridgeTransfer entity for a transfer away from this chain
     ...
     * @returns BridgeTransfer
     */
    transferOut(pool, route, destination, amount, transactionID = null, updateMetrics = true) {
        this.countTransferOut();
        return this.transfer(pool, route, destination, amount, true, transactionID, updateMetrics);
    }
    /**
     ...
     * @returns BridgeTransfer
     */
    nonEVMTransferIn(pool, route, source, amount, transactionID = null, updateMetrics = true) {
        this.countTransferIn();
        return this.transfer(pool, route, source, amount, false, transactionID, updateMetrics);
    }
    /**
     * Creates a BridgeTransfer entity for a transfer arriving to this chain
     * and updates the volumes at PoolRoute, Pool and Protocol.
     ...
     * @returns BridgeTransfer
     */
    transferIn(pool, route, source, amount, transactionID = null, updateMetrics = true) {
        this.countTransferIn();
        return this.transfer(pool, route, source, amount, false, transactionID, updateMetrics);
    }
    transfer(pool, route, counterparty, amount, isOutgoing, transactionID, updateMetrics) {
        const _pool = pool.pool;
        const token = this.tokens.getOrCreateToken(graph_ts_1.Address.fromBytes(_pool.inputToken));
        ...
        pool.trackTransfer(transfer, route, isOutgoing ? enums_1.TransactionType.TRANSFER_OUT : enums_1.TransactionType.TRANSFER_IN);
        return transfer;
    }
    transferBoilerplate() {
    ...
    messageIn(srcChainId, source, data) {
        ...
        return this.message(srcChainId, this.protocol.getCurrentChainID(), source, graph_ts_1.Address.fromBytes(this.account.id), false, data);
    }
    /**
    ...
    messageOut(dstChainId, destination, data) {
        ...
        return this.message(this.protocol.getCurrentChainID(), dstChainId, graph_ts_1.Address.fromBytes(this.account.id), destination, true, data);
    }
...
function inferTransferType(poolType, isOutgoing) {
    if (poolType == enums_1.BridgePoolType.LOCK_RELEASE) {
        return isOutgoing ? enums_1.TransferType.LOCK : enums_1.TransferType.RELEASE;
    }
