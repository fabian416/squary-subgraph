...
function getRewardsPerDay(currentTimestamp, currentBlockNumber, rewardRate, rewardType) {
    ...
    if (currentTimestampI32 - recentSavedTimestamp <=
...
function getStartingBlockRate() {
    ...
    if (configure_1.NetworkConfigs.getNetwork() == constants_1.Network.MAINNET) {
        return graph_ts_1.BigDecimal.fromString("13.39");
    }
    else if (configure_1.NetworkConfigs.getNetwork() == constants_1.Network.ARBITRUM_ONE) {
        return graph_ts_1.BigDecimal.fromString("15");
    }
    else if (configure_1.NetworkConfigs.getNetwork() == constants_1.Network.AURORA) {
        return graph_ts_1.BigDecimal.fromString("1.03");
    }
    else if (configure_1.NetworkConfigs.getNetwork() == constants_1.Network.BSC) {
        return graph_ts_1.BigDecimal.fromString("5");
    }
    else if (configure_1.NetworkConfigs.getNetwork() == constants_1.Network.CELO) {
        return graph_ts_1.BigDecimal.fromString("5");
    }
    else if (configure_1.NetworkConfigs.getNetwork() == constants_1.Network.FANTOM) {
        return graph_ts_1.BigDecimal.fromString("1");
    }
    else if (configure_1.NetworkConfigs.getNetwork() == constants_1.Network.FUSE) {
        return graph_ts_1.BigDecimal.fromString("1");
    }
    else if (configure_1.NetworkConfigs.getNetwork() == constants_1.Network.OPTIMISM) {
        return graph_ts_1.BigDecimal.fromString("12.5");
    }
    else if (configure_1.NetworkConfigs.getNetwork() == constants_1.Network.MATIC) {
        return graph_ts_1.BigDecimal.fromString("2");
    }
    else if (configure_1.NetworkConfigs.getNetwork() == constants_1.Network.XDAI) {
        return graph_ts_1.BigDecimal.fromString("5");
    }
    else if (configure_1.NetworkConfigs.getNetwork() == constants_1.Network.MOONBEAM) {
        return graph_ts_1.BigDecimal.fromString("13.39");
    }
    else if (configure_1.NetworkConfigs.getNetwork() == constants_1.Network.MOONRIVER) {
        return graph_ts_1.BigDecimal.fromString("13.39");
    }
    else if (configure_1.NetworkConfigs.getNetwork() == constants_1.Network.AVALANCHE) {
        return graph_ts_1.BigDecimal.fromString("13.39");
    }
    else if (configure_1.NetworkConfigs.getNetwork() == constants_1.Network.CRONOS) {
...
function getTokenFromRewardToken(tokens, rewardToken) {
    let token = null;
    if (constants_1.GNO_REWARDS.includes(rewardToken)) {
        token = tokens.getOrCreateToken(graph_ts_1.Address.fromString(constants_1.RewardTokens.GNO));
    }
    if (constants_1.OP_REWARDS.includes(rewardToken)) {
        token = tokens.getOrCreateToken(graph_ts_1.Address.fromString(constants_1.RewardTokens.OP));
    }
    if (constants_1.HOP_REWARDS.includes(rewardToken)) {
        token = tokens.getOrCreateToken(graph_ts_1.Address.fromString(constants_1.RewardTokens.HOP));
    }
    if (constants_1.RPL_REWARDS.includes(rewardToken)) {
        if (equalsIgnoreCase(graph_ts_1.dataSource.network(), constants_1.Network.ARBITRUM_ONE))
            token = tokens.getOrCreateToken(graph_ts_1.Address.fromString(constants_1.RewardTokens.rETH_ARB));
        if (equalsIgnoreCase(graph_ts_1.dataSource.network(), constants_1.Network.OPTIMISM))
            token = tokens.getOrCreateToken(graph_ts_1.Address.fromString(constants_1.RewardTokens.rETH_OP));
    }
    if (constants_1.WMATIC_REWARDS.includes(rewardToken)) {
        if (equalsIgnoreCase(graph_ts_1.dataSource.network(), constants_1.Network.MATIC))
            token = tokens.getOrCreateToken(graph_ts_1.Address.fromString(constants_1.RewardTokens.WMATIC_POLYGON));
        if (equalsIgnoreCase(graph_ts_1.dataSource.network(), constants_1.Network.XDAI))
            token = tokens.getOrCreateToken(graph_ts_1.Address.fromString(constants_1.RewardTokens.WMATIC_XDAI));
    }
    if (constants_1.WETH_REWARDS.includes(rewardToken)) {
        token = tokens.getOrCreateToken(graph_ts_1.Address.fromString(constants_1.RewardTokens.WETH));
    }
    return token;
}
function updateWithdrawn(pools, hPools, tokens, poolName, poolSymbol, hPoolName, poolAddress, rewardToken, amount) {
    const token = getTokenFromRewardToken(tokens, rewardToken);
    if (!token)
        return;
    poolAddress =
        configure_1.NetworkConfigs.getPoolAddressFromRewardTokenAddress(rewardToken);
    const pool = pools.loadPool(graph_ts_1.Address.fromString(poolAddress));
    const hPool = hPools.loadPool(graph_ts_1.Bytes.fromHexString(poolAddress.concat("-").concat("1")));
    if (!pool.isInitialized) {
        pool.initialize(poolName, poolSymbol, enums_1.BridgePoolType.LIQUIDITY, token);
    }
    if (!hPool.isInitialized) {
        hPool.initialize(hPoolName, poolSymbol, enums_1.BridgePoolType.LIQUIDITY, token);
    }
    pool.addStakedOutputTokenAmount(amount.times(constants_3.BIGINT_MINUS_ONE));
    hPool.addStakedOutputTokenAmount(amount.times(constants_3.BIGINT_MINUS_ONE));
}
exports.updateWithdrawn = updateWithdrawn;
function updateStaked(pools, hPools, tokens, poolName, poolSymbol, hPoolName, poolAddress, rewardToken, amount) {
    const token = getTokenFromRewardToken(tokens, rewardToken);
    if (!token)
        return;
    poolAddress =
...
function updateRewardsPaid(pools, hPools, tokens, poolName, poolSymbol, hPoolName, poolAddress, event) {
    ...
    const Reward = L2_Reward_1.L2_Reward.bind(event.address);
    const rewardRateCall = Reward.try_rewardRate();
    if (!rewardRateCall.reverted) {
        const rewardRate = rewardRateCall.value.div(constants_3.BIGINT_TWO);
        const dailyEmission = graph_ts_1.BigInt.fromI32(exports.RATE_IN_SECONDS).times(rewardRate);
        pool.setRewardEmissions(constants_3.RewardTokenType.DEPOSIT, token, dailyEmission);
        hPool.setRewardEmissions(constants_3.RewardTokenType.DEPOSIT, token, dailyEmission);
    }
    else {
        graph_ts_1.log.info("Rewards rate call reverted", []);
    }
}
exports.updateRewardsPaid = updateRewardsPaid;
