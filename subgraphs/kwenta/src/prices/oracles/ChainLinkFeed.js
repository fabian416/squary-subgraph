...
const ChainLinkContract_1 = require("../../../generated/FuturesMarketManager2/ChainLinkContract");
...
function getTokenPriceUSDC(tokenAddr, block = null) {
    ...
    if (!result.reverted) {
        const decimals = chainLinkContract.try_decimals(tokenAddr, constants.CHAIN_LINK_USD_ADDRESS);
        if (decimals.reverted) {
            return new types_1.CustomPriceType();
        }
        return types_1.CustomPriceType.initialize(result.value.value1.toBigDecimal(), decimals.value, constants.OracleType.CHAINLINK_FEED);
    }
    return new types_1.CustomPriceType();
}
