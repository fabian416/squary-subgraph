...
class Pool {
    ...
    addRevenueByToken(token, protocolSide, supplySide, stakeSide) {
        const protocolAmountUSD = this.protocol.pricer.getAmountValueUSD(token, protocolSide, this.protocol.event.block);
        const supplyAmountUSD = this.protocol.pricer.getAmountValueUSD(token, supplySide, this.protocol.event.block);
        const stakeAmountUSD = this.protocol.pricer.getAmountValueUSD(token, stakeSide, this.protocol.event.block);
    ...
    addUsdPremium(amountUSD, transactionType) {
    ...
    addPremiumByToken(token, amount, transactionType) {
        const premiumUSD = this.protocol.pricer.getAmountValueUSD(token, amount, this.protocol.event.block);
        if (transactionType == enums_1.TransactionType.DEPOSIT) {
     ...
     */
    setLongOpenInterest(amount, price) {
        const longOpenInterest = amount;
        const totalOpenInterest = this.pool.totalOpenInterest.plus(longOpenInterest.minus(this.pool.longOpenInterest));
        this.pool.totalOpenInterest = totalOpenInterest;
        this.pool.longOpenInterest = longOpenInterest;
        const longOpenInterestUSD = (0, numbers_1.bigIntToBigDecimal)(longOpenInterest).times((0, numbers_1.bigIntToBigDecimal)(price));
        const totalOpenInterestUSD = (0, numbers_1.bigIntToBigDecimal)(totalOpenInterest).times((0, numbers_1.bigIntToBigDecimal)(price));
        const amountChangeUSD = longOpenInterestUSD.minus(this.pool.longOpenInterestUSD);
     ...
     */
    updateLongOpenInterestUSD(amountChangeUSD, isIncrease) {
        if (isIncrease) {
     ...
     * @param price  {BigInt} The last token price
     */
    setShortOpenInterest(amount, price) {
        const shortOpenInterest = amount;
        const totalOpenInterest = this.pool.totalOpenInterest.plus(shortOpenInterest.minus(this.pool.shortOpenInterest));
        this.pool.totalOpenInterest = totalOpenInterest;
        this.pool.shortOpenInterest = shortOpenInterest;
        const shortOpenInterestUSD = (0, numbers_1.bigIntToBigDecimal)(shortOpenInterest).times((0, numbers_1.bigIntToBigDecimal)(price));
        const totalOpenInterestUSD = (0, numbers_1.bigIntToBigDecimal)(totalOpenInterest).times((0, numbers_1.bigIntToBigDecimal)(price));
        const amountChangeUSD = shortOpenInterestUSD.minus(this.pool.shortOpenInterestUSD);
        this.pool.totalOpenInterestUSD = totalOpenInterestUSD;
     ...
     * @param amountChangeUSD {BigDecimal} The value to add to the pool's openInterest in USD.
     */
    updateShortOpenInterestUSD(amountChangeUSD, isIncrease) {
        if (isIncrease) {
            this.pool.totalOpenInterestUSD =
                this.pool.totalOpenInterestUSD.plus(amountChangeUSD);
    ...
    refreshOutputTokenPriceUSD() {
        ...
        let price = this.protocol.pricer.getTokenPrice(token, this.protocol.event.block);
        if (price.equals(constants.BIGDECIMAL_ZERO) &&
            !this.pool.outputTokenSupply.equals(constants.BIGINT_ZERO)) {
            price = this.pool.totalValueLockedUSD.div(this.pool
    ...
    refreshInputTokenWeights() {
        ...
        for (let idx = 0; idx < this.pool.inputTokens.length; idx++) {
            ...
            inputTokenWeights.push((0, numbers_1.safeDivide)(inputTokenTVL, this.pool.totalValueLockedUSD).times(constants.BIGDECIMAL_HUNDRED));
        }
    ...
    addInputTokenBalances(amounts, updateMetrics = true) {
        if (amounts.length != this.pool.inputTokenBalances.length) {
            return;
        }
        const newBalances = (0, arrays_1.addArrays)(this.pool.inputTokenBalances, amounts);
    ...
    setInputTokensById(inputTokenIds) {
    ...
    addInputToken(token, newTokenBalance = constants.BIGINT_ZERO) {
    ...
    setRewardEmissions(type, token, amount) {
        ...
        const amountUSD = this.protocol.pricer.getAmountValueUSD(token, amount, this.protocol.event.block);
     ...
     * @param amount The amount of the token
     */
    addCumulativeVolumeByTokenAmount(token, amount) {
        const amountUSD = this.protocol.pricer.getAmountValueUSD(token, amount, this.protocol.event.block);
        const tokenIndex = this.pool.inputTokens.indexOf(token.id);
        if (tokenIndex == -1)
            return;
