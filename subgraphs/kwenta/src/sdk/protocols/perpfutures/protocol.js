...
class Perpetual {
    ...
    static load(conf, pricer, event) {
        const protocolId = graph_ts_1.Address.fromString(conf.getID());
        let protocol = schema_1.DerivPerpProtocol.load(protocolId);
        if (!protocol) {
            protocol = new schema_1.DerivPerpProtocol(protocolId);
    ...
    addTransaction(type) {
        ...
        else if (type == enums_1.TransactionType.COLLATERAL_IN) {
            this.protocol.collateralInCount += 1;
        }
        else if (type == enums_1.TransactionType.COLLATERAL_OUT) {
