...
class Pool {
    ...
    refreshOutputTokenPriceUSD() {
        ...
        if (price.equals(constants.BIGDECIMAL_ZERO) &&
            ...
            for (let i = 0; i < tranchesAddresses.length; i++) {
                ...
                pricesSum = pricesSum.plus(tranche.tvl.div((0, numbers_1.bigIntToBigDecimal)(tranche.totalSupply, constants.DEFAULT_DECIMALS)));
            }
            if (tranchesAddresses.length <= 0)
                price = constants.BIGDECIMAL_ZERO;
            price = pricesSum.div(graph_ts_1.BigDecimal.fromString(tranchesAddresses.length.toString()));
        }
    ...
    addTranche(trancheId) {
        if (!this.pool._tranches)
            return;
    ...
    setBorrowedAssetAmountUSD(amountsUSD) {
    ...
    updateFundingRates() {
        ...
        for (let idx = 0; idx < this.pool.inputTokens.length; idx++) {
            ...
            if (amountUSD.equals(constants.BIGDECIMAL_ZERO)) {
                ...
                fundingRate.push(borrowedAssetAmountUSD
                    .div(amountUSD)
                    .times(graph_ts_1.BigDecimal.fromString("0.00001")));
            }
        }
