 ...
 */
function handleMatch(call) {
    // sellTarget is sell.target (addrs[11])
    let sellTarget = call.inputs.addrs[11];
    if (sellTarget.equals(constants_1.WYVERN_ATOMICIZER_ADDRESS)) {
...
function handleSingleSale(call) {
    ...
    let mergedCallData = (0, utils_1.guardedArrayReplace)(call.inputs.calldataBuy, call.inputs.calldataSell, call.inputs.replacementPatternBuy);
    let decodedTransferResult = (0, helpers_1.decodeSingleNftData)(call, mergedCallData);
    ...
    let strategy = (0, utils_1.getSaleStrategy)(saleKind);
    let priceETH = (0, helpers_1.calcTradePriceETH)(call, paymentToken);
    collectionAddrs.push(collectionAddr);
    if (buyer != call.inputs.addrs[1].toHexString()) {
        graph_ts_1.log.debug("buyMaker/receiver do not match, isBundle: {}, buyMaker: {}, reciever: {}, tx: {}", [
            false.toString(),
    ...
    if (seller != call.inputs.addrs[8].toHexString()) {
        graph_ts_1.log.debug("sellMaker/sender do not match, isBundle: {}, sellMaker: {}, sender: {}, tx: {}", [
            false.toString(),
    ...
    let trade = new schema_1.Trade(tradeID);
    ...
    let dailyTradedItemID = "DAILY_TRADED_ITEM-"
        ...
        .concat(tokenId.toString())
        .concat("-")
        .concat((call.block.timestamp.toI32() / constants_1.SECONDS_PER_DAY).toString());
    ...
    updateCollectionMetrics(call, collectionAddr, buyer, seller, priceETH, newDailyTradedItem, trade.isBundle);
    // Update Marketplace and daily snapshot
    updateMarketplaceMetrics(call, collectionAddrs, buyer, seller, priceETH, dailyTradedItems);
}
function handleBundleSale(call) {
    let dailyTradedItems = 0;
    let collectionAddrs = [];
    ...
    // paymentToken is buyOrder.paymentToken or SellOrder.payment token (addrs[6] or addrs[13])
    let paymentToken = call.inputs.addrs[13];
    let bundlePriceETH = (0, helpers_1.calcTradePriceETH)(call, paymentToken);
    let mergedCallData = (0, utils_1.guardedArrayReplace)(call.inputs.calldataBuy, call.inputs.calldataSell, call.inputs.replacementPatternBuy);
    let decodedTransferResults = (0, helpers_1.decodeBundleNftData)(call, mergedCallData);
    let tradeSize = graph_ts_1.BigInt.fromI32(decodedTransferResults.length).toBigDecimal();
    for (let i = 0; i < decodedTransferResults.length; i++) {
        ...
        collectionAddrs.push(collectionAddr);
        if (strategy == constants_1.SaleStrategy.DUTCH_AUCTION) {
            graph_ts_1.log.debug("dutch auction sale in a bundle sale, transaction: {}", [
                call.transaction.hash.toHexString(),
            ]);
        }
        if (buyer != decodedTransferResults[i].to.toHexString()) {
            graph_ts_1.log.warning("buyMaker/receiver do not match, isBundle: {}, buyMaker: {}, reciever: {}, tx: {}", [
                true.toString(),
                buyer,
        ...
        if (seller != decodedTransferResults[i].from.toHexString()) {
            graph_ts_1.log.warning("sellMaker/sender do not match, isBundle: {}, sellMaker: {}, sender: {}, tx: {}", [
                true.toString(),
                seller,
        ...
        // Update Collection and daily snapshot
        updateCollectionMetrics(call, collectionAddr, buyer, seller, avgTradePriceETH, newDailyTradedItem, trade.isBundle);
    }
    // Update Marketplace and daily snapshot
    updateMarketplaceMetrics(call, collectionAddrs, buyer, seller, bundlePriceETH, dailyTradedItems);
}
function updateCollectionMetrics(call, collectionAddr, buyer, seller, priceETH, newDailyTradedItem, isBundle) {
    let collection = (0, helpers_1.getOrCreateCollection)(collectionAddr);
    ...
    let collectionSnapshot = (0, helpers_1.getOrCreateCollectionDailySnapshot)(collectionAddr, call.block.timestamp);
    ...
    if (!isBundle) {
        collectionSnapshot.dailyMinSalePrice = (0, utils_1.min)(collectionSnapshot.dailyMinSalePrice, priceETH);
        collectionSnapshot.dailyMaxSalePrice = (0, utils_1.max)(collectionSnapshot.dailyMaxSalePrice, priceETH);
    }
...
function updateMarketplaceMetrics(call, collectionAddrs, buyer, seller, priceETH, dailyTradedItems) {
    let marketplace = (0, helpers_1.getOrCreateMarketplace)(configure_1.NetworkConfigs.getMarketplaceAddress());
    ...
    let marketplaceSnapshot = (0, helpers_1.getOrCreateMarketplaceDailySnapshot)(call.block.timestamp);
...
function updateCollectionRevenueMetrics(call, collectionAddr, priceETH, isBundle) {
    let collection = (0, helpers_1.getOrCreateCollection)(collectionAddr);
    let sellSideFeeRecipient = call.inputs.addrs[10];
    if (sellSideFeeRecipient.notEqual(constants_1.NULL_ADDRESS)) {
        // Sell-side order is maker (sale)
        let makerRelayerFee = call.inputs.uints[9];
        let creatorRoyaltyFeePercentage = constants_1.EXCHANGE_MARKETPLACE_FEE.le(makerRelayerFee)
            ? makerRelayerFee
                .minus(constants_1.EXCHANGE_MARKETPLACE_FEE)
                .divDecimal(constants_1.BIGDECIMAL_HUNDRED)
        ...
        let totalRevenueETH = makerRelayerFee
            .toBigDecimal()
            .times(priceETH)
            .div(constants_1.INVERSE_BASIS_POINT);
        let marketplaceRevenueETH = constants_1.EXCHANGE_MARKETPLACE_FEE.le(makerRelayerFee)
            ? constants_1.EXCHANGE_MARKETPLACE_FEE.toBigDecimal()
                .times(priceETH)
                .div(constants_1.INVERSE_BASIS_POINT)
            : constants_1.BIGDECIMAL_ZERO;
        ...
        let creatorRoyaltyFeePercentage = constants_1.EXCHANGE_MARKETPLACE_FEE.le(takerRelayerFee)
            ? takerRelayerFee
                .minus(constants_1.EXCHANGE_MARKETPLACE_FEE)
                .divDecimal(constants_1.BIGDECIMAL_HUNDRED)
            : constants_1.BIGDECIMAL_ZERO;
        ...
        let totalRevenueETH = takerRelayerFee
            ...
            .times(priceETH)
            .div(constants_1.INVERSE_BASIS_POINT);
        let marketplaceRevenueETH = constants_1.EXCHANGE_MARKETPLACE_FEE.le(takerRelayerFee)
            ? constants_1.EXCHANGE_MARKETPLACE_FEE.toBigDecimal()
                .times(priceETH)
                .div(constants_1.INVERSE_BASIS_POINT)
            : constants_1.BIGDECIMAL_ZERO;
...
function updateMarketplaceRevenueMetrics(call, priceETH) {
    let marketplace = (0, helpers_1.getOrCreateMarketplace)(configure_1.NetworkConfigs.getMarketplaceAddress());
    let sellSideFeeRecipient = call.inputs.addrs[10];
    if (sellSideFeeRecipient.notEqual(constants_1.NULL_ADDRESS)) {
        // Sell-side order is maker (sale)
        ...
        marketplace.marketplaceRevenueETH = marketplace.marketplaceRevenueETH.plus(marketplaceRevenueETH);
