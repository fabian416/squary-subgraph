...
exports.takeUsageMetricsHourlySnapshot = exports.takeUsageMetricsDailySnapshot = exports.takeSnapshots = void 0;
...
function takeSnapshots(event, pool) {
    ...
    if (pool._lastDailySnapshotTimestamp
        ...
        if (pool._lastDailySnapshotTimestamp.equals(constants_1.BIGINT_ZERO)) {
    ...
    if (helper.lastDailyFinancialsTimestamp
        .plus(constants_1.SECONDS_PER_DAY_BI)
        .lt(event.block.timestamp)) {
        takeFinancialsDailySnapshot(event, helper);
    }
    if (helper.lastDailyUsageTimestamp
        .plus(constants_1.SECONDS_PER_DAY_BI)
        .lt(event.block.timestamp)) {
        takeUsageMetricsDailySnapshot(event, helper);
    }
    if (helper.lastHourlyUsageTimestamp
        .plus(constants_1.SECONDS_PER_HOUR_BI)
        .lt(event.block.timestamp)) {
...
function takePoolDailySnapshot(event, pool) {
    const days = event.block.timestamp.toI32() / constants_1.SECONDS_PER_DAY;
    const id = pool.id.concatI32(days);
    const snapshot = new schema_1.LiquidityPoolDailySnapshot(id);
    const protocol = (0, protocol_1.getOrCreateOpynProtocol)();
    snapshot.days = days;
    ...
    const prevSnapshotDays = pool._lastDailySnapshotTimestamp.toI32() / constants_1.SECONDS_PER_DAY;
    const prevSnapshot = schema_1.LiquidityPoolDailySnapshot.load(pool.id.concatI32(prevSnapshotDays));
    if (prevSnapshot) {
        snapshot.dailySupplySideRevenueUSD =
            pool.cumulativeSupplySideRevenueUSD.minus(prevSnapshot.cumulativeSupplySideRevenueUSD);
        snapshot.dailyProtocolSideRevenueUSD =
            pool.cumulativeProtocolSideRevenueUSD.minus(prevSnapshot.cumulativeProtocolSideRevenueUSD);
        snapshot.dailyTotalRevenueUSD = pool.cumulativeTotalRevenueUSD.minus(prevSnapshot.cumulativeTotalRevenueUSD);
        snapshot.dailyEntryPremiumUSD = pool.cumulativeEntryPremiumUSD.minus(prevSnapshot.cumulativeEntryPremiumUSD);
        snapshot.dailyExitPremiumUSD = pool.cumulativeExitPremiumUSD.minus(prevSnapshot.cumulativeExitPremiumUSD);
        snapshot.dailyTotalPremiumUSD = pool.cumulativeTotalPremiumUSD.minus(prevSnapshot.cumulativeTotalPremiumUSD);
        snapshot.dailyDepositPremiumUSD = pool.cumulativeDepositPremiumUSD.minus(prevSnapshot.cumulativeDepositPremiumUSD);
        snapshot.dailyWithdrawPremiumUSD = pool.cumulativeWithdrawPremiumUSD.minus(prevSnapshot.cumulativeWithdrawPremiumUSD);
        snapshot.dailyTotalLiquidityPremiumUSD =
            pool.cumulativeTotalLiquidityPremiumUSD.minus(prevSnapshot.cumulativeTotalLiquidityPremiumUSD);
        ...
        snapshot.dailyVolumeUSD = pool.cumulativeVolumeUSD.minus(prevSnapshot.cumulativeVolumeUSD);
        snapshot.dailyVolumeByTokenAmount = [
            pool.cumulativeVolumeByTokenAmount[0].minus(prevSnapshot.cumulativeVolumeByTokenAmount[0]),
        ];
        snapshot.dailyVolumeByTokenUSD = [
            pool.cumulativeVolumeByTokenUSD[0].minus(prevSnapshot.cumulativeVolumeByTokenUSD[0]),
        ];
        snapshot.dailyDepositedVolumeByTokenAmount = [
            pool.cumulativeDepositedVolumeByTokenAmount[0].minus(prevSnapshot.cumulativeDepositedVolumeByTokenAmount[0]),
        ];
        snapshot.dailyDepositedVolumeByTokenUSD = [
            pool.cumulativeDepositedVolumeByTokenUSD[0].minus(prevSnapshot.cumulativeDepositedVolumeByTokenUSD[0]),
        ];
        snapshot.dailyDepositedVolumeUSD = pool.cumulativeDepositedVolumeUSD.minus(prevSnapshot.cumulativeDepositedVolumeUSD);
        snapshot.dailyWithdrawVolumeUSD = pool.cumulativeWithdrawVolumeUSD.minus(prevSnapshot.cumulativeWithdrawVolumeUSD);
        snapshot.dailyWithdrawVolumeByTokenAmount = [
            pool.cumulativeWithdrawVolumeByTokenAmount[0].minus(prevSnapshot.cumulativeWithdrawVolumeByTokenAmount[0]),
        ];
        snapshot.dailyWithdrawVolumeByTokenUSD = [
            pool.cumulativeWithdrawVolumeByTokenUSD[0].minus(prevSnapshot.cumulativeWithdrawVolumeByTokenUSD[0]),
        ];
        snapshot.dailyClosedVolumeUSD = pool.cumulativeClosedVolumeUSD.minus(prevSnapshot.cumulativeClosedVolumeUSD);
        snapshot.dailyExerciseVolumeUSD = pool.cumulativeExercisedVolumeUSD.minus(prevSnapshot.cumulativeExerciseVolumeUSD);
    }
    snapshot.save();
    pool._lastDailySnapshotTimestamp = event.block.timestamp;
    pool.save();
}
function takePoolHourlySnapshot(event, pool) {
    const hours = event.block.timestamp.toI32() / constants_1.SECONDS_PER_HOUR;
    const id = pool.id.concatI32(hours);
    const snapshot = new schema_1.LiquidityPoolHourlySnapshot(id);
    const protocol = (0, protocol_1.getOrCreateOpynProtocol)();
    ...
    snapshot.cumulativeWithdrawVolumeByTokenUSD =
        snapshot.hourlyWithdrawVolumeByTokenUSD =
            pool.cumulativeWithdrawVolumeByTokenUSD;
    const prevSnapshotHours = pool._lastHourlySnapshotTimestamp.toI32() / constants_1.SECONDS_PER_HOUR;
    const prevSnapshot = schema_1.LiquidityPoolHourlySnapshot.load(pool.id.concatI32(prevSnapshotHours));
    if (prevSnapshot) {
        snapshot.hourlySupplySideRevenueUSD =
            pool.cumulativeSupplySideRevenueUSD.minus(prevSnapshot.cumulativeSupplySideRevenueUSD);
        snapshot.hourlyProtocolSideRevenueUSD =
            pool.cumulativeProtocolSideRevenueUSD.minus(prevSnapshot.cumulativeProtocolSideRevenueUSD);
        snapshot.hourlyTotalRevenueUSD = pool.cumulativeTotalRevenueUSD.minus(prevSnapshot.cumulativeTotalRevenueUSD);
        snapshot.hourlyEntryPremiumUSD = pool.cumulativeEntryPremiumUSD.minus(prevSnapshot.cumulativeEntryPremiumUSD);
        snapshot.hourlyExitPremiumUSD = pool.cumulativeExitPremiumUSD.minus(prevSnapshot.cumulativeExitPremiumUSD);
        snapshot.hourlyTotalPremiumUSD = pool.cumulativeTotalPremiumUSD.minus(prevSnapshot.cumulativeTotalPremiumUSD);
        snapshot.hourlyDepositPremiumUSD = pool.cumulativeDepositPremiumUSD.minus(prevSnapshot.cumulativeDepositPremiumUSD);
        snapshot.hourlyWithdrawPremiumUSD = pool.cumulativeWithdrawPremiumUSD.minus(prevSnapshot.cumulativeWithdrawPremiumUSD);
        snapshot.hourlyTotalLiquidityPremiumUSD =
            pool.cumulativeTotalLiquidityPremiumUSD.minus(prevSnapshot.cumulativeTotalLiquidityPremiumUSD);
        snapshot.hourlyVolumeUSD = pool.cumulativeVolumeUSD.minus(prevSnapshot.cumulativeVolumeUSD);
        ...
        snapshot.hourlyDepositVolumeByTokenAmount = [
            pool.cumulativeDepositedVolumeByTokenAmount[0].minus(prevSnapshot.cumulativeDepositVolumeByTokenAmount[0]),
        ];
        snapshot.hourlyDepositVolumeByTokenUSD = [
            pool.cumulativeDepositedVolumeByTokenUSD[0].minus(prevSnapshot.cumulativeDepositVolumeByTokenUSD[0]),
        ];
        snapshot.hourlyDepositVolumeUSD = pool.cumulativeDepositedVolumeUSD.minus(prevSnapshot.cumulativeDepositVolumeUSD);
        snapshot.hourlyWithdrawVolumeUSD = pool.cumulativeWithdrawVolumeUSD.minus(prevSnapshot.cumulativeWithdrawVolumeUSD);
        snapshot.hourlyWithdrawVolumeByTokenAmount = [
            pool.cumulativeWithdrawVolumeByTokenAmount[0].minus(prevSnapshot.cumulativeWithdrawVolumeByTokenAmount[0]),
        ];
        snapshot.hourlyWithdrawVolumeByTokenUSD = [
...
function takeFinancialsDailySnapshot(event, helper) {
    const protocol = (0, protocol_1.getOrCreateOpynProtocol)();
    const days = event.block.timestamp.toI32() / constants_1.SECONDS_PER_DAY;
    const id = graph_ts_1.Bytes.fromI32(days);
    const snapshot = new schema_1.FinancialsDailySnapshot(id);
    snapshot.days = days;
    snapshot.protocol = protocol.id;
    snapshot.timestamp = event.block.timestamp;
    ...
    const prevSnapshot = schema_1.FinancialsDailySnapshot.load(helper.lastDailyFinancialsSnapshot);
    if (prevSnapshot) {
        snapshot.dailyVolumeUSD = protocol.cumulativeVolumeUSD.minus(prevSnapshot.cumulativeVolumeUSD);
        snapshot.dailyExercisedVolumeUSD =
            protocol.cumulativeExercisedVolumeUSD.minus(prevSnapshot.cumulativeExercisedVolumeUSD);
        snapshot.dailyClosedVolumeUSD = protocol.cumulativeClosedVolumeUSD.minus(prevSnapshot.cumulativeClosedVolumeUSD);
        snapshot.dailySupplySideRevenueUSD =
            protocol.cumulativeSupplySideRevenueUSD.minus(prevSnapshot.cumulativeSupplySideRevenueUSD);
        snapshot.dailyProtocolSideRevenueUSD =
            protocol.cumulativeProtocolSideRevenueUSD.minus(prevSnapshot.cumulativeProtocolSideRevenueUSD);
        snapshot.dailyTotalRevenueUSD = protocol.cumulativeTotalRevenueUSD.minus(prevSnapshot.cumulativeTotalRevenueUSD);
        snapshot.dailyEntryPremiumUSD = protocol.cumulativeEntryPremiumUSD.minus(prevSnapshot.cumulativeEntryPremiumUSD);
        snapshot.dailyExitPremiumUSD = protocol.cumulativeExitPremiumUSD.minus(prevSnapshot.cumulativeExitPremiumUSD);
        snapshot.dailyTotalPremiumUSD = protocol.cumulativeTotalPremiumUSD.minus(prevSnapshot.cumulativeTotalPremiumUSD);
        snapshot.dailyDepositPremiumUSD =
            protocol.cumulativeDepositPremiumUSD.minus(prevSnapshot.cumulativeDepositPremiumUSD);
        snapshot.dailyWithdrawPremiumUSD =
            protocol.cumulativeWithdrawPremiumUSD.minus(prevSnapshot.cumulativeWithdrawPremiumUSD);
        snapshot.dailyTotalLiquidityPremiumUSD =
            protocol.cumulativeTotalLiquidityPremiumUSD.minus(prevSnapshot.cumulativeTotalLiquidityPremiumUSD);
        snapshot.dailyPutsMintedCount =
...
function takeUsageMetricsDailySnapshot(event, helper) {
    const days = event.block.timestamp.toI32() / constants_1.SECONDS_PER_DAY;
    const id = graph_ts_1.Bytes.fromI32(days);
    const protocol = (0, protocol_1.getOrCreateOpynProtocol)();
    const activityHelper = (0, usage_1.getOrCreateActivityHelper)();
    const usageMetrics = new schema_1.UsageMetricsDailySnapshot(id);
...
function takeUsageMetricsHourlySnapshot(event, helper) {
    const hours = event.block.timestamp.toI32() / constants_1.SECONDS_PER_HOUR;
...
function getOrCreateSnapshotHelper() {
    let snapshotHelper = schema_1._ProtocolSnapshotHelper.load(constants_1.SnapshotHelperID);
    if (!snapshotHelper) {
        snapshotHelper = new schema_1._ProtocolSnapshotHelper(constants_1.SnapshotHelperID);
        snapshotHelper.lastDailyFinancialsTimestamp = constants_1.BIGINT_ZERO;
        snapshotHelper.lastDailyUsageTimestamp = constants_1.BIGINT_ZERO;
        snapshotHelper.lastHourlyUsageTimestamp = constants_1.BIGINT_ZERO;
        snapshotHelper.lastDailyFinancialsSnapshot = graph_ts_1.Bytes.fromI32(0);
        snapshotHelper.lastDailyUsageSnapshot = graph_ts_1.Bytes.fromI32(0);
        snapshotHelper.lastHourlyUsageSnapshot = graph_ts_1.Bytes.fromI32(0);
    }
