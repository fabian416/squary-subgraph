...
function initRegistry() {
    graph_ts_1.log.debug("Initializing token prices registry", []);
    if (schema_1._TokenPrice.load("loaded") != null && schema_1.Token.load("loaded") != null) {
        return;
    }
    const prices = [
        [18806, 10.0, 4.61],
    ...
    if (!schema_1._TokenPrice.load("loaded")) {
        const priceLoaded = new schema_1._TokenPrice("loaded");
        priceLoaded.save();
        for (let i = 0; i < prices.length; i++) {
            let tokenPrice = schema_1._TokenPrice.load(prices[i][0].toString());
            if (!tokenPrice) {
                tokenPrice = new schema_1._TokenPrice(prices[i][0].toString());
                tokenPrice.cosmos = graph_ts_1.BigDecimal.fromString(prices[i][1].toString());
                tokenPrice.osmosis = graph_ts_1.BigDecimal.fromString(prices[i][2].toString());
                tokenPrice.save();
            }
        }
    }
    ...
    if (!schema_1.Token.load("loaded")) {
        const tokenLoaded = new schema_1.Token("loaded");
        ...
        for (let i = 0; i < tokens.length; i++) {
            ...
            if (!token) {
                ...
                token.decimals = graph_ts_1.BigInt.fromString(tokens[i][3].toString()).toI32();
