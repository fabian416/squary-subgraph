...
exports.updateMetricsAfterSwap = exports.updatePoolMetrics = exports.updateUsageMetrics = exports.updateFinancials = exports.updateProtocolTVL = void 0;
...
function updatePoolTVL(event, assetAddress, protocolLockedValue, updateValue) {
    ...
    if (!pool._ignore) {
        ...
        pool.totalValueLockedUSD = (0, numbers_1.tokenAmountToUSDAmount)(token, _asset.cash);
        pool.save();
        protocolLockedValue = protocolLockedValue.plus(pool.totalValueLockedUSD);
    }
    return protocolLockedValue;
}
function updateProtocolTVL(event, assetAddress) {
    ...
    if (!assetAddress.equals(constants_1.ZERO_ADDRESS)) {
        ...
        protocolLockedValue = constants_1.BIGDECIMAL_ZERO;
        for (let i = 0; i < protocol.pools.length; i++) {
            protocolLockedValue = updatePoolTVL(event, graph_ts_1.Address.fromString(protocol.pools[i]), protocolLockedValue, false);
        }
    }
...
function updateUsageMetrics(event, user, transactionType) {
    const protocol = (0, getters_1.getOrCreateDexAmm)();
    const hourlyUsageSnapshot = (0, getters_1.getOrCreateHourlyUsageMetricSnapshot)(event);
    const dailyUsageSnapshot = (0, getters_1.getOrCreateDailyUsageMetricSnapshot)(event);
    let account = schema_1.Account.load(user.toHexString());
    if (!account) {
        account = new schema_1.Account(user.toHexString());
        account.save();
        protocol.cumulativeUniqueUsers += 1;
        protocol.save();
    }
    const hours = (0, datetime_1.getHours)(event.block.timestamp.toI64()).toString();
    const hourlyAccountId = "hourly-".concat(user.toHexString().concat("-").concat(hours));
    let hourlyAccount = schema_1.ActiveAccount.load(hourlyAccountId.toString());
    if (!hourlyAccount) {
        hourlyAccount = new schema_1.ActiveAccount(hourlyAccountId);
        hourlyAccount.save();
        hourlyUsageSnapshot.hourlyActiveUsers += 1;
    }
    const days = (0, datetime_1.getDays)(event.block.timestamp.toI64()).toString();
    const dailyAccountId = "daily-".concat(user.toHexString().concat("-").concat(days));
    let dailyAccount = schema_1.ActiveAccount.load(dailyAccountId);
    if (!dailyAccount) {
        dailyAccount = new schema_1.ActiveAccount(dailyAccountId);
        dailyAccount.save();
    ...
    switch (transactionType) {
        ...
        case constants_1.TransactionType.WITHDRAW:
            ...
            break;
        case constants_1.TransactionType.SWAP:
...
function updatePoolMetrics(event, assetAddress, tokenAddress) {
    const pool = (0, getters_1.getOrCreateAssetPool)(event, assetAddress, event.address, tokenAddress);
    const hourlySnapshot = (0, getters_1.getOrCreateLiquidityPoolHourlySnapshot)(event, assetAddress, event.address, tokenAddress);
    const dailySnapshot = (0, getters_1.getOrCreateLiquidityPoolDailySnapshot)(event, assetAddress, event.address, tokenAddress);
...
function calculateSwapVolume(swap) {
    return swap.amountInUSD.plus(swap.amountOutUSD).div(constants_1.BIGDECIMAL_TWO);
}
function calculateSwapFeeInTokenAmount(event, swap, haircutRate) {
    // Fee calculation
    // feeInTokenAmount = (haircut_rate * actual_amount) / (1 - haircut_rate)
    haircutRate = haircutRate.div((0, numbers_1.exponentToBigDecimal)(18));
    const n = haircutRate.times(swap.amountOut.toBigDecimal());
    const d = graph_ts_1.BigDecimal.fromString("1").minus(haircutRate);
    return n.div(d);
}
function calculateSwapFeeInUsd(event, swap, haircutRate) {
    // Fee calculation
    ...
    const feeToken = (0, getters_1.getOrCreateToken)(event, graph_ts_1.Address.fromString(swap.tokenOut));
    const amountUsd = (0, numbers_1.tokenAmountToUSDAmount)(feeToken, graph_ts_1.BigInt.fromString(feeInTokenAmount.toString().split(".")[0]));
    return amountUsd;
}
function updateMetricsAfterSwap(event, poolAddress, fromAssetAddress, toAssetAddress, fromTokenAddress, toTokenAddress, fromTokenAmount, actualToTokenAmount, sender, to) {
    const protocol = (0, getters_1.getOrCreateDexAmm)();
    const fromPool = (0, getters_1.getOrCreateAssetPool)(event, fromAssetAddress, event.address, fromTokenAddress);
    const toPool = (0, getters_1.getOrCreateAssetPool)(event, toAssetAddress, event.address, toTokenAddress);
    ...
    const inputToken = (0, getters_1.getOrCreateToken)(event, fromTokenAddress);
    const outputToken = (0, getters_1.getOrCreateToken)(event, toTokenAddress);
    const amountInUsd = (0, numbers_1.tokenAmountToUSDAmount)(inputToken, fromTokenAmount);
    const amountOutUsd = (0, numbers_1.tokenAmountToUSDAmount)(outputToken, actualToTokenAmount);
    ...
    swap.save();
    // swap fee metrics
    const poolMetrics = (0, getters_1.getOrCreateLiquidityPoolParamsHelper)(event, poolAddress);
    const swapFeeUsd = calculateSwapFeeInUsd(event, swap, poolMetrics.HaircutRate);
    const supplySideFee = (0, numbers_1.exponentToBigDecimal)(18).minus(poolMetrics.RetentionRatio).times(swapFeeUsd);
    ...
    toPool.save();
    const toPoolHourlySnapshot = (0, getters_1.getOrCreateLiquidityPoolHourlySnapshot)(event, toAssetAddress, event.address, graph_ts_1.Address.fromString(swap.tokenOut));
    // block number and timestamp
    ...
    const toPoolDailySnapshot = (0, getters_1.getOrCreateLiquidityPoolDailySnapshot)(event, toAssetAddress, event.address, graph_ts_1.Address.fromString(swap.tokenOut));
    // block number and timestamp
    ...
    const fromPoolHourlySnapshot = (0, getters_1.getOrCreateLiquidityPoolHourlySnapshot)(event, graph_ts_1.Address.fromString(swap.fromPool), event.address, graph_ts_1.Address.fromString(swap.tokenIn));
    // block number and timestamp
    ...
    const fromPoolDailySnapshot = (0, getters_1.getOrCreateLiquidityPoolDailySnapshot)(event, graph_ts_1.Address.fromString(swap.fromPool), event.address, graph_ts_1.Address.fromString(swap.tokenIn));
    // block number and timestamp
