...
const TraderJoeRouter_1 = require("./routers/TraderJoeRouter");
function getUsdPricePerToken(tokenAddr) {
    ...
    // TraderJoe Router
    const traderJoePrice = (0, TraderJoeRouter_1.getPriceUsdc)(tokenAddr, network);
    if (!traderJoePrice.reverted) {
        graph_ts_1.log.warning("[TraderJoeRouter] tokenAddress: {}, Price: {}", [
            tokenAddr.toHexString(),
            traderJoePrice.usdPrice.div(traderJoePrice.decimalsBaseTen).toString(),
        ]);
        return traderJoePrice;
    }
    graph_ts_1.log.warning("[Oracle] Failed to Fetch Price with TraderJoe will try sushiswap next, tokenAddr: {}", [
        tokenAddr.toHexString(),
    ]);
    // 7. SushiSwap Router
...
function getUsdPrice(tokenAddr, amount) {
    ...
    graph_ts_1.log.warning("[Oracle] Failed to Fetch Price, tokenAddr: {}", [tokenAddr.toHexString()]);
