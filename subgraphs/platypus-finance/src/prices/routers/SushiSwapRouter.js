...
const SushiSwapRouter_1 = require("../../../generated/Pool/SushiSwapRouter");
function isLpToken(tokenAddress, network) {
    if (tokenAddress.equals(constants.WHITELIST_TOKENS_MAP.get(network).get(constants.NETWORK_BASE_TOKEN_MAP.get(network)[0]))) {
        return false;
    }
...
function getPriceFromRouter(token0Address, token1Address, network) {
    ...
    let amountOutArray;
    graph_ts_1.log.warning("[Sushiswap Router]Calling: AmountIn: {} using path: {}!", [
        amountIn.toString(),
