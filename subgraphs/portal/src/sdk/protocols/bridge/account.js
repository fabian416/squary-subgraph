...
class Account {
    ...
    liquidityDeposit(pool, amount, updateMetrics = true) {
        const _pool = pool.pool;
        const token = this.tokens.getOrCreateToken(graph_ts_1.Address.fromBytes(_pool.inputToken));
        const id = idFromEvent(enums_1.TransactionType.LIQUIDITY_DEPOSIT, this.event);
    ...
    liquidityWithdraw(pool, amount, updateMetrics = true) {
        const _pool = pool.pool;
        const token = this.tokens.getOrCreateToken(graph_ts_1.Address.fromBytes(_pool.inputToken));
        const id = idFromEvent(enums_1.TransactionType.LIQUIDITY_WITHDRAW, this.event);
...
function idFromEvent(eventType, event) {
    return graph_ts_1.Bytes.fromUTF8(eventType)
