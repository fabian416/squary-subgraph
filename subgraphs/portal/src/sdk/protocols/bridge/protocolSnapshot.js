...
class ProtocolSnapshot {
    ...
    takeFinancialsDailySnapshot() {
        ...
        const previous = schema_1.FinancialsDailySnapshot.load(helper.lastDailyFinancialsSnapshot);
        if (previous) {
            ...
            volumeInDelta = snapshot.cumulativeVolumeInUSD.minus(previous.cumulativeVolumeInUSD);
            volumeOutDelta = snapshot.cumulativeVolumeOutUSD.minus(previous.cumulativeVolumeOutUSD);
            netVolumeDelta = snapshot.cumulativeNetVolumeUSD.minus(previous.cumulativeNetVolumeUSD);
        }
    ...
    takeUsageDailySnapshot() {
        ...
        const previous = schema_1.UsageMetricsDailySnapshot.load(helper.lastDailyUsageSnapshot);
        if (previous) {
            transactionDelta =
    ...
    takeUsageHourlySnapshot() {
        ...
        let messageSentDelta = snapshot.cumulativeMessageSentCount;
        let messageReceivedDelta = snapshot.cumulativeMessageReceivedCount;
        const previous = schema_1.UsageMetricsHourlySnapshot.load(helper.lastHourlyUsageSnapshot);
        if (previous) {
            transactionDelta =
                snapshot.cumulativeTransactionCount -
                    previous.cumulativeTransactionCount;
