...
function updateUsageMetrics(block, from) {
    ...
    usageMetricsHourlySnapshot.cumulativeUniqueUsers =
        protocol.cumulativeUniqueUsers;
    const dailyActiveAccountId = (block.timestamp.toI64() / constants_1.SECONDS_PER_DAY)
        .toString()
        .concat("-")
        .concat(accountId);
    ...
    const hourlyActiveAccountId = (block.timestamp.toI64() / constants_1.SECONDS_PER_HOUR)
        .toString()
        .concat("-")
        .concat(accountId);
