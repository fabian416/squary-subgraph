...
const UniswapPair_1 = require("../../../generated/UniswapV2Factory/UniswapPair");
const UniswapRouter_1 = require("../../../generated/UniswapV2Factory/UniswapRouter");
const UniswapFactory_1 = require("../../../generated/UniswapV2Factory/UniswapFactory");
...
function getPriceFromRouter(token0Address, token1Address, block = null) {
    ...
    for (let idx = 0; idx < routerAddresses.length; idx++) {
        ...
        if (block && router.startBlock.gt(block.number))
            continue;
        const uniswapForkRouter = UniswapRouter_1.UniswapRouter.bind(router.address);
    ...
    const amountOutBigDecimal = amountOut
        ...
        .toBigDecimal();
    const priceFromRouter = types_1.CustomPriceType.initialize(amountOutBigDecimal, config.usdcTokenDecimals().toI32(), constants.OracleType.UNISWAP_FORKS_ROUTER);
    const routerContract = UniswapRouter_1.UniswapRouter.bind(routerAddress);
    const factoryAddress = utils.readValue(routerContract.try_factory(), constants.NULL.TYPE_ADDRESS);
    if (factoryAddress.equals(constants.NULL.TYPE_ADDRESS))
        return priceFromRouter;
    const factoryContract = UniswapFactory_1.UniswapFactory.bind(factoryAddress);
    const tokenPair = utils.readValue(factoryContract.try_getPair(token0Address, wethAddress), constants.NULL.TYPE_ADDRESS);
...
function getLpTokenTotalLiquidityUsdc(tokenAddress, block = null) {
    ...
    if (reserve0.notEqual(constants.BIGINT_ZERO) ||
        ...
        const liquidity0 = reserve0
            .toBigDecimal()
            .div(constants.BIGINT_TEN.pow(token0Decimals.toI32()).toBigDecimal())
            .times(token0Price.usdPrice);
        const liquidity1 = reserve1
            .toBigDecimal()
            .div(constants.BIGINT_TEN.pow(token1Decimals.toI32()).toBigDecimal())
...
function getLpTokenLiquidityUsdc(lpAddress, wethAddress, block = null) {
    const uniSwapPair = UniswapPair_1.UniswapPair.bind(lpAddress);
    ...
    if (wethReserves.notEqual(constants.BIGINT_ZERO)) {
        const liquidityUSDC = utils
            .bigIntToBigDecimal(wethReserves, wethDecimals.toI32() - constants.DEFAULT_USDC_DECIMALS)
            .times(wethPrice.usdPrice);
        return types_1.CustomPriceType.initialize(liquidityUSDC, constants.DEFAULT_USDC_DECIMALS, constants.OracleType.UNISWAP_FORKS_ROUTER);
    }
    return new types_1.CustomPriceType();
}
