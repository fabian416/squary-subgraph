...
class PoolSnapshot {
    ...
    takeSnapshots() {
        ...
        if (this.pool
            ._lastDailySnapshotTimestamp.plus(constants_1.SECONDS_PER_DAY_BI)
            .lt(this.event.block.timestamp)) {
            this.takeDailySnapshot();
        }
        if (this.pool
            ._lastHourlySnapshotTimestamp.plus(constants_1.SECONDS_PER_HOUR_BI)
            .lt(this.event.block.timestamp)) {
            this.takeHourlySnapshot();
        }
    }
    isInitialized() {
        return !!(this.pool._lastDailySnapshotTimestamp &&
            this.pool._lastHourlySnapshotTimestamp);
    }
    takeHourlySnapshot() {
        const event = this.event;
        const pool = this.pool;
        const hour = event.block.timestamp.div(constants_1.SECONDS_PER_HOUR_BI).toI32();
        const previousHour = pool
            ._lastHourlySnapshotTimestamp.div(constants_1.SECONDS_PER_HOUR_BI)
            .toI32();
        ...
        for (let i = 0; i < pool.routes.length; i++) {
            routeSnapshots.push(this.takeRouteSnapshot(event, snapshot.id, pool.routes[i], previousID));
        }
        ...
        snapshot.netCumulativeVolume = pool.cumulativeVolumeIn.minus(pool.cumulativeVolumeOut);
        snapshot.netCumulativeVolumeUSD = pool.cumulativeVolumeInUSD.minus(pool.cumulativeVolumeOutUSD);
        // deltas
        ...
        if (previous) {
            supplySideRevenueDelta = snapshot.cumulativeSupplySideRevenueUSD.minus(previous.cumulativeSupplySideRevenueUSD);
            protocolSideRevenueDelta =
                snapshot.cumulativeProtocolSideRevenueUSD.minus(previous.cumulativeProtocolSideRevenueUSD);
            totalRevenueDelta = snapshot.cumulativeTotalRevenueUSD.minus(previous.cumulativeTotalRevenueUSD);
            volumeInDelta = snapshot.cumulativeVolumeIn.minus(previous.cumulativeVolumeIn);
            volumeInUSDDelta = snapshot.cumulativeVolumeInUSD.minus(previous.cumulativeVolumeInUSD);
            volumeOutDelta = snapshot.cumulativeVolumeOut.minus(previous.cumulativeVolumeOut);
            volumeOutUSDDelta = snapshot.cumulativeVolumeOutUSD.minus(previous.cumulativeVolumeOutUSD);
            netVolumeDelta = snapshot.netCumulativeVolume.minus(previous.netCumulativeVolume);
            netVolumeUSDDelta = snapshot.netCumulativeVolumeUSD.minus(previous.netCumulativeVolumeUSD);
        }
    ...
    takeDailySnapshot() {
        const event = this.event;
        const pool = this.pool;
        const day = event.block.timestamp.div(constants_1.SECONDS_PER_DAY_BI).toI32();
        const previousDay = pool
            ._lastDailySnapshotTimestamp.div(constants_1.SECONDS_PER_DAY_BI)
            .toI32();
    ...
    takeRouteSnapshot(event, snapshotID, routeId, previousSnapshotID) {
        ...
        const previous = schema_1.PoolRouteSnapshot.load(poolRouteSnapshotID(routeId, previousSnapshotID));
        if (previous) {
            intervalVolumeIn = snapshot.cumulativeVolumeIn.minus(previous.cumulativeVolumeIn);
            intervalVolumeInUSD = snapshot.cumulativeVolumeInUSD.minus(previous.cumulativeVolumeInUSD);
            intervalVolumeOut = snapshot.cumulativeVolumeOut.minus(previous.cumulativeVolumeOut);
            intervalVolumeOutUSD = snapshot.cumulativeVolumeOutUSD.minus(previous.cumulativeVolumeOutUSD);
        }
