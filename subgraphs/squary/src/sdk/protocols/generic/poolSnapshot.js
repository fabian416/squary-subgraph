...
class PoolSnapshot {
    ...
    takeSnapshots() {
        if (!this.pool.lastUpdateTimestamp)
            return;
        const snapshotDayID = this.pool.lastUpdateTimestamp.toI32() / constants_1.SECONDS_PER_DAY;
        const snapshotHourID = this.pool.lastUpdateTimestamp.toI32() / constants_1.SECONDS_PER_HOUR;
    ...
    takeHourlySnapshot(hour) {
        const snapshot = new schema_1.PoolHourlySnapshot(this.pool.id.concatI32(hour));
        const previousSnapshot = schema_1.PoolHourlySnapshot.load(this.pool.id.concatI32(this.pool.lastSnapshotHourID));
        snapshot.hours = hour;
    ...
    takeDailySnapshot(day) {
        const snapshot = new schema_1.PoolDailySnapshot(this.pool.id.concatI32(day));
        const previousSnapshot = schema_1.PoolDailySnapshot.load(this.pool.id.concatI32(this.pool.lastSnapshotDayID));
        snapshot.day = day;
        snapshot.protocol = this.pool.protocol;
        snapshot.pool = this.pool.id;
        snapshot.timestamp = this.event.block.timestamp;
