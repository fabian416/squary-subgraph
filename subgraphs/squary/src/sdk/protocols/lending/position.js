...
class PositionManager {
    ...
    addPosition(event, asset, protocol, newBalance, transactionType, priceUSD, principal = null) {
    ...
    subtractPosition(event, protocol, newBalance, transactionType, priceUSD, principal = null) {
        const positionCounter = schema_1._PositionCounter.load(this.counterID);
        if (!positionCounter) {
    ...
    snapshotPosition(event, priceUSD) {
        const snapshot = new schema_1.PositionSnapshot(this.position.id.concat("-")
            .concat(event.transaction.hash.toHexString())
            .concat("-")
            .concat(event.logIndex.toString()));
        const token = new token_1.TokenManager(this.position.asset, event);
        const mantissaFactorBD = (0, constants_1.exponentToBigDecimal)(token.getDecimals());
    ...
    dailyActivePosition(counter, event, protocol) {
        ...
        const currentDay = event.block.timestamp.toI32() / constants_1.SECONDS_PER_DAY;
        if (lastDay == currentDay) {
            return;
        }
        // this is a new active position
        const snapshots = new snapshots_1.SnapshotManager(event, protocol, this.market);
