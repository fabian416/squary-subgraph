...
class Account {
    ...
    getAmountUSD(tokens, amounts) {
        ...
        for (let idx = 0; idx < tokens.length; idx++) {
            ...
            amountUSD = amountUSD.plus(this.protocol.getTokenPricer().getAmountValueUSD(token, amounts[idx]));
        }
        return amountUSD;
    }
    ...
    trackActivity(activityType, liquidator = null) {
        ...
        if (liquidator)
            accountId = liquidator.toHexString();
    ...
    liquidate(pool, asset, collateralToken, amountLiquidated, liquidator, liquidatee, position, profitUSD, updateMetrics = true) {
        ...
        liquidate.amountUSD = this.protocol
            .getTokenPricer()
            .getAmountValueUSD(this.tokens.getOrCreateToken(collateralToken), amountLiquidated);
