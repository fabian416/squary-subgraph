...
exports.initActivityHelper = exports.ProtocolSnapshot = void 0;
...
class ProtocolSnapshot {
    constructor(protocol, event) {
        ...
        this.dailyActivityHelper = initActivityHelper(graph_ts_1.Bytes.fromUTF8(constants.ActivityInterval.DAILY.concat("-").concat(this.dayID.toString())));
        this.hourlyActivityHelper = initActivityHelper(graph_ts_1.Bytes.fromUTF8(constants.ActivityInterval.HOURLY.concat("-").concat(this.hourID.toString())));
        this.takeSnapshots();
    }
    ...
    takeSnapshots() {
        if (!this.protocol._lastUpdateTimestamp) {
            graph_ts_1.log.error("[isInitialized] cannot create snapshots, protocol: {} not initialized", [this.protocol.id.toHexString()]);
            return;
        }
        const snapshotDayID = this.protocol._lastUpdateTimestamp.toI32() / constants.SECONDS_PER_DAY;
        const snapshotHourID = this.protocol._lastUpdateTimestamp.toI32() / constants.SECONDS_PER_HOUR;
        if (snapshotDayID != this.dayID) {
            this.takeFinancialsDailySnapshot(snapshotDayID);
            this.takeUsageDailySnapshot(snapshotDayID);
            this.protocol._lastSnapshotDayID = graph_ts_1.BigInt.fromI32(snapshotDayID);
            this.protocol.save();
        }
        if (snapshotHourID != this.hourID) {
            this.takeUsageHourlySnapshot(snapshotHourID);
            this.protocol._lastSnapshotHourID = graph_ts_1.BigInt.fromI32(snapshotDayID);
            this.protocol.save();
        }
    }
    takeFinancialsDailySnapshot(day) {
        const snapshot = new schema_1.FinancialsDailySnapshot(graph_ts_1.Bytes.fromI32(day));
        const previousSnapshot = schema_1.FinancialsDailySnapshot.load(graph_ts_1.Bytes.fromI32(this.protocol._lastSnapshotDayID.toI32()));
        snapshot.days = day;
        ...
        snapshot.dailyVolumeUSD = previousSnapshot
            ? this.protocol.cumulativeVolumeUSD.minus(previousSnapshot.cumulativeVolumeUSD)
        ...
        snapshot.dailyInflowVolumeUSD = previousSnapshot
            ? this.protocol.cumulativeInflowVolumeUSD.minus(previousSnapshot.cumulativeInflowVolumeUSD)
            : this.protocol.cumulativeInflowVolumeUSD;
        snapshot.dailyClosedInflowVolumeUSD = previousSnapshot
            ? this.protocol.cumulativeClosedInflowVolumeUSD.minus(previousSnapshot.cumulativeClosedInflowVolumeUSD)
            : this.protocol.cumulativeClosedInflowVolumeUSD;
        snapshot.dailyOutflowVolumeUSD = previousSnapshot
            ? this.protocol.cumulativeOutflowVolumeUSD.minus(previousSnapshot.cumulativeOutflowVolumeUSD)
        ...
        snapshot.dailySupplySideRevenueUSD = previousSnapshot
            ? this.protocol.cumulativeSupplySideRevenueUSD.minus(previousSnapshot.cumulativeSupplySideRevenueUSD)
            : this.protocol.cumulativeSupplySideRevenueUSD;
        snapshot.dailyProtocolSideRevenueUSD = previousSnapshot
            ? this.protocol.cumulativeProtocolSideRevenueUSD.minus(previousSnapshot.cumulativeProtocolSideRevenueUSD)
            : this.protocol.cumulativeProtocolSideRevenueUSD;
        snapshot.dailyStakeSideRevenueUSD = previousSnapshot
            ? this.protocol.cumulativeStakeSideRevenueUSD.minus(previousSnapshot.cumulativeStakeSideRevenueUSD)
            : this.protocol.cumulativeStakeSideRevenueUSD;
        snapshot.dailyTotalRevenueUSD = previousSnapshot
            ? this.protocol.cumulativeTotalRevenueUSD.minus(previousSnapshot.cumulativeTotalRevenueUSD)
        ...
        snapshot.dailyEntryPremiumUSD = previousSnapshot
            ? this.protocol.cumulativeEntryPremiumUSD.minus(previousSnapshot.cumulativeEntryPremiumUSD)
            : this.protocol.cumulativeEntryPremiumUSD;
        snapshot.dailyExitPremiumUSD = previousSnapshot
            ? this.protocol.cumulativeExitPremiumUSD.minus(previousSnapshot.cumulativeExitPremiumUSD)
            : this.protocol.cumulativeExitPremiumUSD;
        snapshot.dailyTotalPremiumUSD = previousSnapshot
            ? this.protocol.cumulativeTotalPremiumUSD.minus(previousSnapshot.cumulativeTotalPremiumUSD)
        ...
        snapshot.dailyDepositPremiumUSD = previousSnapshot
            ? this.protocol.cumulativeDepositPremiumUSD.minus(previousSnapshot.cumulativeDepositPremiumUSD)
            : this.protocol.cumulativeDepositPremiumUSD;
        snapshot.dailyWithdrawPremiumUSD = previousSnapshot
            ? this.protocol.cumulativeWithdrawPremiumUSD.minus(previousSnapshot.cumulativeWithdrawPremiumUSD)
            : this.protocol.cumulativeWithdrawPremiumUSD;
        snapshot.dailyTotalLiquidityPremiumUSD = previousSnapshot
            ? this.protocol.cumulativeTotalLiquidityPremiumUSD.minus(previousSnapshot.cumulativeTotalLiquidityPremiumUSD)
    ...
    takeUsageDailySnapshot(day) {
        const activity = initActivityHelper(graph_ts_1.Bytes.fromUTF8(constants.ActivityInterval.DAILY.concat("-").concat(day.toString())));
        const snapshot = new schema_1.UsageMetricsDailySnapshot(graph_ts_1.Bytes.fromI32(day));
        const previousSnapshot = schema_1.UsageMetricsDailySnapshot.load(graph_ts_1.Bytes.fromI32(this.protocol._lastSnapshotDayID.toI32()));
        snapshot.days = day;
        snapshot.protocol = this.protocol.id;
        snapshot.timestamp = this.event.block.timestamp;
        ...
        snapshot.dailyLongPositionCount = previousSnapshot
            ? max(this.protocol.longPositionCount - previousSnapshot.longPositionCount, 0)
        ...
        snapshot.dailyShortPositionCount = previousSnapshot
            ? max(this.protocol.shortPositionCount -
                previousSnapshot.shortPositionCount, 0)
        ...
        snapshot.dailyOpenPositionCount = previousSnapshot
            ? max(this.protocol.openPositionCount - previousSnapshot.openPositionCount, 0)
    ...
    takeUsageHourlySnapshot(hour) {
        const activity = initActivityHelper(graph_ts_1.Bytes.fromUTF8(constants.ActivityInterval.HOURLY.concat("-").concat(hour.toString())));
...
function initActivityHelper(id) {
    let activityHelper = schema_1._ActivityHelper.load(id);
    if (!activityHelper) {
        activityHelper = new schema_1._ActivityHelper(id);
...
exports.initActivityHelper = initActivityHelper;
