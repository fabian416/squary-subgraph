...
const UniswapPair_1 = require("../../../generated/LPStaking_0/UniswapPair");
const UniswapRouter_1 = require("../../../generated/LPStaking_0/UniswapRouter");
...
function getTokenPriceUSDC(tokenAddress) {
...
function getPriceFromRouterUSDC(tokenAddress, usdcAddress) {
    return getPriceFromRouter(tokenAddress, usdcAddress);
}
exports.getPriceFromRouterUSDC = getPriceFromRouterUSDC;
function getPriceFromRouter(token0Address, token1Address) {
    const config = utils.getConfig();
    const ethAddress = config.ethAddress();
    ...
    for (let idx = 0; idx < routerAddresses.length; idx++) {
        ...
        const uniswapForkRouter = UniswapRouter_1.UniswapRouter.bind(routerAddress);
    ...
    const feeBips = graph_ts_1.BigInt.fromI32(constants_1.INT_THIRTY);
    const amountOutBigDecimal = amountOut
        ...
        .toBigDecimal();
    return types_1.CustomPriceType.initialize(amountOutBigDecimal, config.usdcTokenDecimals().toI32());
}
exports.getPriceFromRouter = getPriceFromRouter;
function getLpTokenPriceUsdc(tokenAddress) {
    const uniSwapPair = UniswapPair_1.UniswapPair.bind(tokenAddress);
    const totalLiquidity = getLpTokenTotalLiquidityUsdc(tokenAddress);
...
function getLpTokenTotalLiquidityUsdc(tokenAddress) {
    const uniSwapPair = UniswapPair_1.UniswapPair.bind(tokenAddress);
