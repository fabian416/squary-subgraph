...
class CustomEventType {
    constructor() {
        this.block = new graph_ts_1.ethereum.Block(graph_ts_1.Bytes.empty(), graph_ts_1.Bytes.empty(), graph_ts_1.Bytes.empty(), graph_ts_1.Address.fromString(constants_1.ZERO_ADDRESS), graph_ts_1.Bytes.empty(), graph_ts_1.Bytes.empty(), graph_ts_1.Bytes.empty(), constants_1.BIGINT_ZERO, constants_1.BIGINT_ZERO, constants_1.BIGINT_ZERO, constants_1.BIGINT_ZERO, constants_1.BIGINT_ZERO, constants_1.BIGINT_ZERO, null, null);
        this.transaction = new graph_ts_1.ethereum.Transaction(graph_ts_1.Bytes.empty(), constants_1.BIGINT_ZERO, graph_ts_1.Address.fromString(constants_1.ZERO_ADDRESS), null, constants_1.BIGINT_ZERO, constants_1.BIGINT_ZERO, constants_1.BIGINT_ZERO, graph_ts_1.Bytes.empty(), constants_1.BIGINT_ZERO);
...
class SDK {
    ...
    static initialize(config, pricer, tokenInitializer, event) {
        if (event instanceof graph_ts_1.ethereum.Event) {
            const customEvent = CustomEventType.initialize(event.block, event.transaction, event.logIndex, event);
            return new SDK(config, pricer, tokenInitializer, customEvent);
        }
        if (event instanceof graph_ts_1.ethereum.Call) {
            const customEvent = CustomEventType.initialize(event.block, event.transaction, constants_1.BIGINT_ZERO);
            return new SDK(config, pricer, tokenInitializer, customEvent);
        }
