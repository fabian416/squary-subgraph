...
function getRewardsPerDay(currentTimestamp, currentBlockNumber, rewardRate, rewardType) {
    ...
    if (windowWidth == constants_1.INT_ZERO) {
        if (circularBuffer.nextIndex >= circularBuffer.bufferSize) {
            ...
            blocks[circularBuffer.nextIndex + constants_1.INT_ONE] = currentBlockNumberI32;
            circularBuffer.nextIndex += constants_1.INT_TWO;
        }
        circularBuffer.save();
    ...
    if (circularBuffer.nextIndex == constants_1.INT_ZERO) {
        recentSavedTimestamp = blocks[circularBuffer.bufferSize - constants_1.INT_TWO];
    }
    else {
        recentSavedTimestamp = blocks[circularBuffer.nextIndex - constants_1.INT_TWO];
    }
    ...
    const network = graph_ts_1.dataSource.network().toUpperCase().replace("-", "_");
    if (network != constants_1.Network.ARBITRUM_ONE) {
        ...
        const windowBlocksCount = graph_ts_1.BigDecimal.fromString((currentBlockNumberI32 -
            blocks[circularBuffer.windowStartIndex + constants_1.INT_ONE]).toString());
...
function getOrCreateCircularBuffer() {
    ...
    if (!circularBuffer) {
        ...
        for (let i = constants_1.INT_ZERO; i < exports.BUFFER_SIZE; i += constants_1.INT_TWO) {
...
function getStartingBlockRate() {
    ...
    if (network == constants_1.Network.MAINNET) {
        ...
        return graph_ts_1.BigDecimal.fromString("12");
    }
    ...
    else if (network == constants_1.Network.BASE) {
