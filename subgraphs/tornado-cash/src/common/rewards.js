...
function getRewardsPerDay(currentTimestamp, currentBlockNumber, rewardRate, rewardType) {
    ...
    let windowSecondsCount = graph_ts_1.BigDecimal.fromString((currentTimestampI32 - blocks[circularBuffer.windowStartIndex]).toString());
    // Wideness of the window in blocks.
    let windowBlocksCount = graph_ts_1.BigDecimal.fromString((currentBlockNumberI32 - blocks[circularBuffer.windowStartIndex + constants_2.INT_ONE]).toString());
    // Estimate block speed for the window in seconds.
    let unnormalizedBlockSpeed = exports.WINDOW_SIZE_SECONDS_BD.div(windowSecondsCount).times(windowBlocksCount);
    // block speed converted to specified rate.
    let normalizedBlockSpeed = exports.RATE_IN_SECONDS_BD.div(exports.WINDOW_SIZE_SECONDS_BD).times(unnormalizedBlockSpeed);
...
function getStartingBlockRate() {
    ...
    let network = graph_ts_1.dataSource.network().toUpperCase();
