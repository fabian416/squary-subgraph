...
const MasterChefApeswap_1 = require("../../../../../generated/MasterChef/MasterChefApeswap");
...
function handleReward(event, pid, amount) {
    const poolContract = MasterChefApeswap_1.MasterChefApeswap.bind(event.address);
    ...
    const poolRewardTokenRateBigDecimal = new graph_ts_1.BigDecimal(poolRewardTokenRate);
    // Based on the emissions rate for the pool, calculate the rewards per day for the pool.
    const rewardTokenPerDay = (0, rewards_1.getRewardsPerDay)(event.block.timestamp, event.block.number, poolRewardTokenRateBigDecimal, masterChef.rewardTokenInterval);
