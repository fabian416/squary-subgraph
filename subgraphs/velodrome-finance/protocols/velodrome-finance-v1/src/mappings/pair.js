...
function handleSwap(event) {
    ...
    (0, metrics_1.updateFinancials)(protocol, event);
    const factoryContract = PairFactory_1.PairFactory.bind(graph_ts_1.Address.fromString(constants_2.FACTORY_ADDRESS));
...
function handleTransfer(event) {
    ...
    if (event.params.to.toHexString() == constants_1.ZERO_ADDRESS &&
        event.params.amount.equals(constants_1.BIGINT_THOUSAND) &&
        pool.outputTokenSupply == constants_1.BIGINT_ZERO) {
        return;
    }
    // mints
    if (event.params.from.toHexString() == constants_1.ZERO_ADDRESS) {
        (0, handlers_1.handleTransferMint)(event, pool, event.params.amount, event.params.to.toHexString());
    }
    ...
    if (event.params.to.toHexString() == constants_1.ZERO_ADDRESS &&
        ...
        (0, handlers_1.handleTransferBurn)(event, pool, event.params.amount, event.params.from.toHexString());
    }
}
