...
function getUsdPricePerToken(tokenAddr) {
    ...
    if (!yearnLensPrice.reverted) {
        graph_ts_1.log.debug("[YearnLensOracle] tokenAddress: {}, Price: {}", [
    ...
    if (!chainLinkPrice.reverted) {
        graph_ts_1.log.debug("[ChainLinkFeed] tokenAddress: {}, Price: {}", [
            tokenAddr.toHexString(),
    ...
    if (!calculationsCurvePrice.reverted) {
        graph_ts_1.log.debug("[CalculationsCurve] tokenAddress: {}, Price: {}", [
            tokenAddr.toHexString(),
    ...
    if (!calculationsSushiSwapPrice.reverted) {
        graph_ts_1.log.debug("[CalculationsSushiSwap] tokenAddress: {}, Price: {}", [
            tokenAddr.toHexString(),
    ...
    if (!curvePrice.reverted) {
        graph_ts_1.log.debug("[CurveRouter] tokenAddress: {}, Price: {}", [
            tokenAddr.toHexString(),
    ...
    if (!uniswapPrice.reverted) {
        graph_ts_1.log.debug("[UniswapRouter] tokenAddress: {}, Price: {}", [
            tokenAddr.toHexString(),
    ...
    if (!sushiswapPrice.reverted) {
        graph_ts_1.log.debug("[SushiSwapRouter] tokenAddress: {}, Price: {}", [
            tokenAddr.toHexString(),
            sushiswapPrice.usdPrice.div(sushiswapPrice.decimalsBaseTen).toString(),
        ]);
        return sushiswapPrice;
    }
    graph_ts_1.log.info("[Oracle] Failed to Fetch Price, tokenAddr: {}", [tokenAddr.toHexString()]);
    return new types_1.CustomPriceType();
}
