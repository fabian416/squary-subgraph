...
function updateSPUserPositionBalances(event, sp, depositor, newBalance) {
    ...
    const deltaUSD = (0, token_1.getVSTTokenPrice)(event).times((0, numbers_1.bigIntToBigDecimal)(delta));
    if (delta.gt(constants_1.BIGINT_ZERO)) {
        (0, event_1.createDeposit)(event, sp, delta, deltaUSD, depositor);
...
function updateUserPositionBalances(event, trove) {
    const market = (0, market_1.getOrCreateMarket)(graph_ts_1.Address.fromString(trove.asset));
    const account = (0, account_1.getOrCreateAccount)(graph_ts_1.Address.fromString(trove.owner));
